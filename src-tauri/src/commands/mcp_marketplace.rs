...
/// Filtered, sorted, paginated card query — the single browse path.
///
/// The catalog is ~21k rows across every source, so an unpaginated read cannot
/// be the browse path: it would move the whole registry across IPC on every
/// keystroke and filter it in the renderer's memory (audit A.3-d/A.3-e).
/// [`McpServerPage`] carries `total` alongside the page so a UI can show
/// "60 of 21363" without a second call.
#[tauri::command]
