...
use skillstar_projects::projects::{agents as agent_profile, sync};
...
pub async fn unlink_all_skills_from_agent(agent_id: String) -> Result<u32, AppError> {
    ...
    let result = sync::unlink_all_skills_from_agent(&agent_id).map_err(|e| {
...
pub async fn batch_link_skills_to_agent(
    ...
    let result = sync::batch_link_skills_to_agent(&skill_names, &agent_id)
...
pub async fn unlink_skill_from_agent(skill_name: String, agent_id: String) -> Result<(), AppError> {
    ...
    let result = sync::unlink_skill_from_agent(&skill_name, &agent_id).map_err(|e| {
