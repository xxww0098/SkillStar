...
use crate::core::infra::{daily_log, paths};
...
pub fn read_quota_store() -> Result<QuotaStore> {
    let states = super::provider_states::load()?;
    Ok(states.quotas)
}

/// Persist quota store (delegated to provider_states).
fn write_quota_store(store: &QuotaStore) -> Result<()> {
    let mut states = super::provider_states::load().unwrap_or_default();
    states.quotas = store.clone();
    super::provider_states::save(&states)
}
...
async fn fetch_quota_from_endpoint(
    ...
    let base_url = super::health::resolve_provider_url(provider, app_id).unwrap_or_default();
