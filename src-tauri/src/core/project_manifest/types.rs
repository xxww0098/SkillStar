...
pub enum ProjectDeployMode {
    /// Symlink (Unix) or symlink / junction (Windows via [`crate::core::infra::fs_ops::create_symlink`]).
    #[default]
    Symlink,
...
pub(crate) fn deploy_skill_auto(source: &Path, target: &Path) -> Result<()> {
    let was_copy = crate::core::infra::fs_ops::create_symlink_or_copy(source, target)?;
    if was_copy {
        tracing::info!(
            target: "sync",
...
pub(crate) fn prune_deploy_modes_for_agents(
...
pub(crate) fn ensure_project_root_exists(project_path: &str) -> Result<PathBuf> {
