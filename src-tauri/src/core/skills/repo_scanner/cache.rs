...
/// 1. Clone with `--filter=blob:none --depth 1 --no-checkout` — only tree metadata.
/// 2. Use `git ls-tree` to discover all `SKILL.md` locations.
/// 3. Configure sparse-checkout for only skill directories.
/// 4. `git checkout` materializes just those directories.
///
/// Falls back to a full shallow clone if sparse operations fail.
...
/// Compact sibling skill dirs to their common parent.
///
