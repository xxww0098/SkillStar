...
use super::{
    git_ops, installed_skill, lockfile, project_manifest, repo_scanner, security_scan, sync,
    update_checker,
};
...
pub fn update_skill(name: &str) -> Result<SkillUpdateOutcome> {
    ...
    if !path.exists() && !crate::core::infra::fs_ops::is_link(&path) {
    ...
    for skill_name in &affected_skills {
        ...
        security_scan::invalidate_skill_cache(skill_name);
    }
