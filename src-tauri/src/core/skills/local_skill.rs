...
pub fn is_local_skill(name: &str) -> bool {
    ...
    let Ok(resolved) = crate::core::infra::fs_ops::read_link_resolved(&skill_path) else {
        return false;
    };

    let local_dir = crate::core::infra::paths::local_skills_dir();
...
pub fn create(name: &str, content: Option<&str>) -> Result<Skill> {
    ...
    crate::core::infra::fs_ops::create_symlink(&skill_local_path, &skill_hub_path)
...
pub fn adopt_existing_dir(name: &str, source_dir: &Path) -> Result<PathBuf> {
    ...
    if let Err(err) = crate::core::infra::fs_ops::create_symlink(&skill_local_path, &skill_hub_path)
        .with_context(|| format!("Failed to create hub symlink for '{}'", name))
    {
...
pub fn reconcile_hub_symlinks() {
    ...
    for entry in entries.flatten() {
        ...
        if let Err(e) = crate::core::infra::fs_ops::create_symlink(&path, &hub_path) {
            warn!(
                target: "local_skill",
                skill = %name,
                error = %e,
...
pub fn delete(name: &str) -> Result<()> {
    ...
    let hub_path = hub_dir.join(name);
    if hub_path.symlink_metadata().is_ok() {
        if crate::core::infra::fs_ops::is_link(&hub_path) {
            crate::core::infra::fs_ops::remove_symlink(&hub_path)
...
pub fn migrate_existing() -> Result<u32> {
    ...
    for entry in entries.flatten() {
        ...
        if crate::core::infra::fs_ops::is_link(&path) || !meta.is_dir() {
            continue;
        }
...
fn migrate_single_skill(src: &Path, dest: &Path) -> Result<()> {
    ...
    crate::core::infra::fs_ops::create_symlink(dest, src)
