...
fn store_path() -> PathBuf {
    crate::core::infra::paths::packs_path()
}
...
pub fn install_pack(repo_dir: &Path, source: &str, repo_url: &str) -> Result<Vec<String>> {
    ...
    for skill in &manifest.skills {
        ...
        crate::core::infra::fs_ops::create_symlink(&src_path, &temp_dir)
    ...
    for (name, _temp_dir, target_dir) in &staged {
        if target_dir.exists() && !crate::core::infra::fs_ops::is_link(target_dir) {
            // Existing non-symlink skill conflicts — abort
            // Clean up all staged temp dirs
            for (_, td, _) in &staged {
                let _ = crate::core::infra::fs_ops::remove_symlink(td);
            }
            bail!(
    ...
    for (name, temp_dir, target_dir) in &staged {
        ...
        if crate::core::infra::fs_ops::is_link(target_dir) || target_dir.exists() {
            if crate::core::infra::fs_ops::is_link(target_dir) {
                let _ = crate::core::infra::fs_ops::remove_symlink(target_dir);
            } else {
    ...
    let tree_hash = crate::core::git::ops::compute_tree_hash(repo_dir).unwrap_or_default();
...
pub fn remove_pack(name: &str) -> Result<Vec<String>> {
    ...
    for skill in &pack.skills {
        ...
        if crate::core::infra::fs_ops::is_link(&skill_path) || skill_path.exists() {
            if crate::core::infra::fs_ops::is_link(&skill_path) {
                match crate::core::infra::fs_ops::remove_symlink(&skill_path) {
...
fn resolve_repo_cache_path(pack: &PackEntry) -> PathBuf {
    ...
    let hub_root = crate::core::infra::paths::hub_root();
    ...
    if let Some(suffix) = configured.strip_prefix(".repos/") {
        let migrated = crate::core::infra::paths::repos_cache_dir().join(suffix);
        if migrated.exists() {
            return migrated;
        }
    }
    ...
    if let Some(suffix) = configured.strip_prefix("repos/") {
        let current = crate::core::infra::paths::repos_cache_dir().join(suffix);
        if current.exists() {
            return current;
        }
    }
...
pub fn doctor_pack(name: &str) -> Result<DoctorReport> {
    ...
    for skill in &pack.skills {
        let skill_path = hub.join(&skill.name);
        if !skill_path.exists() && !crate::core::infra::fs_ops::is_link(&skill_path) {
            missing_skills.push(skill.name.clone());
        } else if crate::core::infra::fs_ops::is_link(&skill_path) {
            let broken = match crate::core::infra::fs_ops::read_link_resolved(&skill_path) {
                Ok(p) => !p.exists(),
                Err(_) => true,
            };
            if broken {
