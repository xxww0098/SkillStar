...
static SCHEMA_READY: LazyLock<()> = LazyLock::new(|| {
    let conn = crate::core::infra::db_pool::translation_pool()
        .get()
...
fn source_hash(source_text: &str) -> String {
    crate::core::infra::util::sha256_hex(source_text.as_bytes())
}

/// Returns true when the result looks genuinely translated for the given target language.
/// This prevents caching an AI "translation" that just echoed the source text back.
fn translation_looks_translated_for_cache(
    ...
    // If the result is byte-for-byte identical to the source, it's clearly not translated.
    if source == result {
        return false;
    }
    ...
    if target_needs_cjk {
        // For CJK targets, the translated result must contain target-script characters.
...
pub fn upsert_translation(
    ...
    // Reject translations that don't appear to be translated (same-language output).
    // For CJK targets, the output must contain CJK/kana/hangul script.
    // For other targets, latin-script is assumed valid.
