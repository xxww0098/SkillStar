...
use crate::core::projects::agents as agent_profile;
...
fn remove_managed_entry_for_overwrite(path: &Path) -> Result<bool> {
    let is_link = crate::core::infra::fs_ops::is_link(path);
...
pub fn toggle_skill_for_agent(skill_name: &str, agent_id: &str, enable: bool) -> Result<()> {
    ...
    tracing::info!(
        ...
        is_link = crate::core::infra::fs_ops::is_link(&target),
    ...
    if enable {
        ...
        crate::core::infra::fs_ops::create_symlink(&skill_path, &target)?;
...
pub fn batch_link_skills_to_agent(skill_names: &[String], agent_id: &str) -> Result<u32> {
    ...
    for name in skill_names {
        ...
        let skill_is_link = crate::core::infra::fs_ops::is_link(&skill_path);
        ...
        if !skill_exists {
            if skill_is_link {
                ...
                let _ = crate::core::infra::fs_ops::remove_link_or_copy(&skill_path);
            } else {
                tracing::warn!(
                    target: "sync",
                    skill = %name,
        ...
        if crate::core::infra::fs_ops::is_link(&target) {
        ...
        match crate::core::infra::fs_ops::create_symlink(&skill_path, &target) {
            Ok(()) => {
                tracing::info!(
                    target: "sync",
                    skill = %name,
...
pub fn create_project_skills(
    ...
    crate::core::project_manifest::add_skills_to_project(
...
pub fn resync_existing_links(skill_name: &str) -> Result<Vec<String>> {
    ...
    for profile in profiles.iter() {
        ...
        if crate::core::infra::fs_ops::is_link(&target) {
            crate::core::infra::fs_ops::remove_symlink(&target)?;
            crate::core::infra::fs_ops::create_symlink(&skill_path, &target)?;
