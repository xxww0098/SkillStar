...
fn config_path() -> PathBuf {
    crate::core::infra::paths::config_dir().join("launch_configs.json")
}
...
fn save_all(configs: &HashMap<String, LaunchConfig>) -> Result<()> {
    ...
    // On Windows, rename() refuses to overwrite an existing file, so remove first.
...
pub fn validate(config: &LaunchConfig) -> Result<(), Vec<String>> {
    ...
    for (id, agent_id) in &panes {
        ...
        if crate::core::terminal_backend::find_cli_binary(agent_id).is_none() {
