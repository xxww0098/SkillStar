        ...
        if !options.translate_latex {
            line = INLINE_LATEX_RE
                .replace_all(&line, |caps: &Captures| {
                    ...
                    let placeholder =
                        next_placeholder("LATEX_INLINE", &mut counters.latex_inline);
...
async fn translate_unique_fragments(
    ...
    let results = futures::stream::iter(unique_texts.iter().cloned().enumerate())
        .map(|(index, text)| {
            ...
            async move {
                if let Ok(Some(cached)) = translation_cache::get_cached_translation_for_provider(
                ...
                if let Err(err) = translation_cache::upsert_translation_for_provider(
                    TranslationKind::SkillSection,
                    &target_language,
                    &text,
                    &translated,
...
pub async fn translate_markdown_with_provider(
    ...
    translate_markdown_with_translator(translator, content, target_language, provider_identity).await
}
