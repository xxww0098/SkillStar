//! NVIDIA NIM translation service.
...
pub struct NvidiaNIMService {
    api_key: String,
    settings: TranslationProviderSettings,
}

impl NvidiaNIMService {
    pub fn new(ai_config: &AiConfig) -> Self {
        Self {
            api_key: ai_config.translation_api.nvidia_key.clone(),
            settings: ai_config.translation_api.nvidia_settings.clone(),
        }
    }
}

impl TranslationProvider for NvidiaNIMService {
    ...
    fn translate(
        ...
        async move {
            if api_key.is_empty() {
                return Err(TranslationError::MissingApiKey("NVIDIA NIM".into()));
            }
            ...
            if !status.is_success() {
                return Err(TranslationError::ApiError("NVIDIA NIM".into(), body_text));
            }
            ...
            Ok(TranslationResult::new(translated, "nvidia", text.len()))
        }
    }
}
