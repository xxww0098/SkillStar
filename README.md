...
### 🃏 技能包编排
将多个 Skill 自由组合成一副"套牌"——例如 `React 专家` + `Git 规范` + `Storybook`——为不同复杂度的项目任务按需一键分发。更重要的是，你精心调教的神级卡组可以**一键生成分享码**发送给好友，对方导入分享码即得完整套牌；多文件 Skill 还能打包成 `.agentskill` 离线包，让你的智慧结晶在团队与社区间自由流转。
