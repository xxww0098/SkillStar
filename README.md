...
- **19 家固定 catalog**：
  - **OAuth 自动同步（6 家）**：Cursor (PKCE + 轮询) · Codex (OpenAI OAuth) · Antigravity (Google OAuth) · Trae (区域感知 CN/SG/US/TTP) · Qoder (Device Flow) · Kiro (PKCE + 本地回调 + AWS usage API)
  - **API Key 自动同步（4 家）**：DeepSeek (`/user/balance`) · 智谱 GLM Coding Plan (`/monitor/usage/quota/limit`，5h + 周窗口 + `data.level`) · MiniMax Token Plan (`/v1/token_plan/remains`) · Kimi (`/v1/users/me/balance`)
   - **手动录入（6 家）**：小米 MiMo · 火山方舟 · 阶跃 Step · 阿里百炼
...
> ⚠️ Cursor / Codex / Antigravity / Trae / Qoder / Kiro 的 OAuth 端点是社区蓝本（cockpit-tools），随官方升级可能失效，失败时可降级为手动录入。
...
| OAuth | tiny_http 1455/1456/1457 + PKCE + JWT exp | Cursor / Codex / Trae / Qoder / Kiro / Antigravity 登录 |
