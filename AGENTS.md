...
- **Shipped Tauri IPC** (remote): `discover_remote_skills`, `list_remote_skills`, `push_skill_to_remote`, `migrate_remote_skill_to_hub`, `delete_remote_skill`, `test_ssh_connection`, `accept_ssh_host_key`, plus host CRUD/import. Phase 2 (not yet implemented): read/edit remote skill content, pull, install-from-URL, per-agent link toggle — internal helpers live in `skillstar-ssh::hub` but are not registered commands.
