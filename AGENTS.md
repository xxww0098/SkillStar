...
- **Connection console**: every SSH command streams progress to the frontend via the `ssh://connect-stream` Tauri event. The crate emits structured `SshProgressEvent`s (phase = dial/handshake/host_key/auth/sftp/scan/done/error, status = start/ok/warn/fail/pending) through a `ProgressSink` trait (`progress.rs`) — kept Tauri-agnostic. Remote skill discovery emits `scan` (not `done`) for "scanning remote for skills…" / result summary so the console sequence stays dial → … → sftp → scan → done. The command layer injects a `TauriProgressSink` that forwards to `app.emit`. Each invocation gets a unique `session_id` so the UI can filter events. The frontend `useConnectStream` hook subscribes and renders a terminal-style console in `RemoteSkillPanel`; when a host-key check is `pending` (first connection), the console pauses with a fingerprint + "trust" button instead of a throwaway toast. Errors also land in the console (phase=error, status=fail) so the user sees why a connection failed, not just a silent retry.
...
- **Remote lifecycle helpers (crate-only)**: `skillstar-ssh::hub` also implements read/write content, pull, install-from-URL, per-agent link toggle, and update checks (atomic `.skillstar.tmp`→`rename` writes; `git -C <content> pull --ff-only` / `git clone --depth 1`; `ln -sfn`/`rm -f` for agent symlinks). These are not yet exposed as Tauri IPC commands — the shipped remote surface is discover/list/push/migrate/delete plus connection test and host-key TOFU.
