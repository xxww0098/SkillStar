...
- 组织私有共享频道由 `skillstar-skills::shared_channels` 拥有。GitHub 数字 repository ID 是跨重命名稳定身份；本地版本化 registry 只保存非敏感描述符和创建状态。GitHub 仓库创建与 selected-repository 授权是可恢复的两阶段流程：远端创建后必须先持久化 pending，再授权并转 active。
