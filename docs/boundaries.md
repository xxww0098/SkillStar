...
- `skillstar-skills::shared_channels` 独占共享频道 GitHub REST 编排、权限投影、版本化 descriptor 与本地登记；生产 gateway 必须使用 `probe_http_client`。`src/features/shared-channels/` 是独立前端 feature，只通过 typed IPC 暴露给 My Skills 组合。
