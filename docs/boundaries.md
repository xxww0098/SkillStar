...
| 项目技能 MCP | 本机 stdio 服务、项目技能推荐与批准编排在 `skillstar_app::project_skills_mcp`。工具参数在 `protocol`，不接收批准字段。不进入 `skillstar_app::mcp` 或 `skillstar_models::mcp`。`rmcp`、`ort` 和 `tokenizers` 只加入 `skillstar-app` | `crates/skillstar-app/src/project_skills_mcp/` |
