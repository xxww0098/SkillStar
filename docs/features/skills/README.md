...
- `skillstar-skills::content` 是技能内容读取、文件枚举、本地创建/删除和嵌套内容目录解析的 facade；Tauri command 不直接组合 hub path、lockfile、Git checkout 与 cache invalidation。内容 facade 对外部输入先执行 `validate_skill_name`，再 canonicalize 有效目录并限制在 SkillStar hub 内；外部符号链接不会被 read/list/open 跟随。只读快照（有效内容根、递归文件清单和确定性内容 hash）服务安装/更新完整性，不再驱动教程生成。
...
- Git/local 安装是一条 [vercel-skills 五步管线](../../decisions.md#d-047技能安装是-vercel-skills-五步管线harness-文件夹是-identity-别名)：解析来源 → 发现 `SKILL.md` → Hub 链所选文件夹 → Agent 目录 symlink（Windows 必要时 copy）→ 调用方选 project/global。`.<harness>/skills/<name>` 与 `skills/<name>/` 是同一 identity；chooser 是 `discovery::resolve_install_skills`。CLI、Tauri、轮播、batch 都进 `skill_install`。
...
- GUI 的 `install_skill` / `install_from_scan` 新安装成功后**不默认部署**到任何 Agent，轮播图 SVG 图标保持未点亮；技能卡 SVG Agent 轮播点某个 harness 图标时走**同一条**安装管线，把 `agentId` 传给 chooser 并仅部署到该 Agent：`install_skill(url, name, agentId)` 优先该 Agent 的 `.<harness>/` 树。Library / Marketplace / Publisher / DetailPanel 传给 `SkillCard` 的 `onInstall` 必须接受 `(url, name, agentId?)` 并原样交给 `installSkill`；丢掉 `agentId` 会让已在 hub 的卡（如 rust-skills、impeccable）点灰图标看起来没反应。已链接图标仍 unlink（`toggle_skill_for_agent`）。`install_skill` 返回的 Skill 必须用 `installed_skill::agent_links_for` 重新读盘，不得只填本次点击的 Agent：前端会用返回值整体替换缓存条目，少填会让其余图标灭掉，轮播看起来像单选按钮。没有 `git_url` 时回退 `onToggleAgent(..., true)`。一次安装只让该图标 pending，不得锁整行。（Cursor → `.cursor/skills/<id>` 或 `.cursor`，DSH → `.dsh/skills/<id>`），然后只部署到该 Agent。**已在 hub、repo-cache 里已有 clone 的未链接点击必须 cache-local**：只扫描现有 checkout，不得 `clone`/`fetch`；`source_folder` 没变就不改 lock。cache 缺失时才允许走慢的 fetch（首次安装）。**同一 Skill 名只保留一条 lock**：`source_folder` 跟随最近一次明确请求的 harness。复用仅当现有 `source_folder` 已经是该次解析到的文件夹；否则从同一 clone 改指向（不二次 clone）。改指向前先把**其他** Agent 的链接钉到它们当前的 payload（不钉正在改指向的那个 Agent）。随后部署到该 Agent 时，若已有 symlink 的 `readlink` 不是本次 hub/harness 正文（上一轮错误部署留下的 stale 链接），必须 link-first 换掉；不得把「路径已存在」当成成功。已钉住的其他 Agent 保持各自 harness 正文。卡片 Install 按钮、Marketplace / Publisher / 详情、CLI `install`（未显式单个 `--agent`）与 scan/reinstall 仍用 catalog 优先规则，但只要存在 catalog 或 harness 副本，就不得把整仓（`source_folder: None`）当作技能。
- repo scan 默认 root-first；仓库根有合法 `SKILL.md` 时首先视作一个技能，也可显式启用全深度 discovery。**例外**：根 `SKILL.md` 与同 identity 的嵌套 catalog（`skills/` / `source/skills/`）或 harness 副本（`.cursor/skills/`、`.dsh/skills/`、`.claude/skills/` 等）并存时，根文件是一层扫描器垫片，不是安装单元。未指定 harness 时优先 `skills/<name>/`；指定 harness 时该 `.<harness>/` 文件夹赢，即使 `skills/` 也存在。**包里没有该 harness 文件夹时按顺序回退，不得 fail-closed，也不得静默 no-op 或整仓安装**：1. 规范 `skills/<name>/` 或 `source/skills/`；2. 已安装则用现有 hub `source_folder`（不改 lock）；3. 同 identity 的另一份 harness 副本。把该 payload 部署到**被点** Agent 的技能目录（DSH → `~/.dsh/skills/<id>`）。只有完全没有嵌套 `SKILL.md` 才失败。完整性扫描（频道发布/校验）把 catalog + harness 同 identity 收成**一个**安装单元，不得把 `.cursor/skills/<id>` 当成第二个 Skill 去改名或报重复；两个独立 catalog 文件夹同名仍 fail-closed。
- Codex 包内技能在 `.agents`（不是 `.codex/skills`）；Antigravity 在 `.agent`。稀疏检出不得按 basename 丢掉同名的 `.agent/...` 与 `.agents/...`；根 `SKILL.md` 若只是垫片，也不得因此改成全量 checkout 而丢掉嵌套文件夹。
- 技能 id 优先使用 frontmatter `name`，再回退目录名；根技能可回退仓库名。
- 安装与扫描必须走同一 frontmatter 质量判定（`skillstar-skills::validation`）：`description` 缺失/非字符串、`name` 超 64 字符、frontmatter 缺失或 YAML 损坏的技能在 repo-scan 安装、share-code embedded、bundle 导出/导入、本地目录采用（`adopt_folder` 与项目导入）处 fail-closed；`description` 超 1024 字符仍记录规范兼容性问题，但只显示黄色警告并允许安装；`description` 中的 `<` / `>` 是合法文本，不产生问题。错误必须列出全部阻塞技能与可行动原因；`name` 缺失（回退目录名）与非 kebab-case 同属咨询级，不阻塞。`DiscoveredSkill.frontmatter_issues` 把稳定 issue code 传给前端，`installable` 直接投影同一后端判定；扫描预览必须逐项显示具体原因、禁止选择红色阻塞项，并允许黄色警告项及其余有效技能安装，不得在前端重建阻塞规则或把所有问题概括为缺少 `name` / `description`。git/local 安装只有一条管线（[D-047](../../decisions.md#d-047技能安装是-vercel-skills-五步管线harness-文件夹是-identity-别名)），不再整仓 clone 到 hub。
- 仓库技能发现对齐生态：priority 容器目录（`skills/`、`.claude/skills/` 等）最多走 3 层，含 SKILL.md 的目录遮蔽其下内容，仓库根保持 1 层；同 identity 去重时 `skills/` 与 `source/skills/` 同为规范目录，高于 harness 副本。`.claude-plugin/marketplace.json` 与 `plugin.json` 声明的本地 `./` 前缀技能路径以声明深度加入扫描（`skills` 字段接受字符串或数组；越界/非 `./` 路径拒绝，manifest 只提供位置不执行插件逻辑）。
...
- 未显式指定 Agent 时，只使用用户已在 Settings 手动启用的 Agent；SkillStar 不探测本机是否安装了对应 binary、桌面应用或配置目录。`-y` 下没有已启用 Agent 时直接报错，不得回退到全部 Agent。显式 `--agent` 与显式 `--all` 始终优先。仅当命令行显式给出**一个** `--agent <id>` 时，安装单元改用该 Agent 的 harness 文件夹；多个 Agent、`--all` 或未写 `--agent` 仍优先 catalog `skills/`。
...
- 本地分歧只能由用户显式解决：一是把当前完整内容保留为独立本地副本后继续更新，二是清理 tracked、untracked 与 ignored 的受管修改后继续更新。默认副本名为 `<原名>.local`，允许编辑；名称冲突时提出 `.local.2` 等非破坏性候选。是否为本地 Skill 只由存储位置与 provenance/type 决定，不解析名称后缀。一次更新发现多个本地分歧时，GUI 在同一个对话框中列出全部 Skill 及各自的可编辑副本名，并由用户一次选择全部保留或全部丢弃；后端仍按逐项事务顺序执行，不为每项重复弹窗。所有页面的单 Skill 更新入口共享同一个选择对话框；CLI 在交互终端提供同样的保留/丢弃/跳过选择，非交互输入保持 fail-closed。
- 对话框逐项展示停止原因（内容已修改 / 缺少安装基线 / 内容读取失败 / 来源已删除 / 来源已删除且内容已丢失）并按界面语言本地化；只有内容已修改才可以称为用户的本地修改，其余原因不得沿用该措辞。`SkillUpdateBlocked.error` 只承载意料之外的失败细节，预期内的停止不重复陈述 `reason`。
- 来源已不再提供某个已安装 Skill 时，它进入独立于本地分歧的停止状态，出口只有「转为本地副本」与「移除」；「丢弃修改」在该状态下必须被后端拒绝，因为没有可恢复的上游内容。移除复用既有 Hub、lockfile、Agent 与 Project 卸载语义，并把被移除的名字通过 `ResolveSkillUpdateResult.uninstalled` 返回，让前端把它从库中删除而不是留下一张失效卡片。被移除的 Skill 无法再驱动自己所在 checkout 的 pull，改由同 checkout 仍存活的 Skill 继续；没有存活项时该次更新到此为止。
- 上游移除与更名在更新检查期就可见，不必等到 pull：`update_state` 为每个 Skill 除 `update_available` 外再保存 `upstream_change`。检查时本地 `HEAD:<source_folder>` 可解析、tracked ref（`origin/HEAD` / pinned `FETCH_HEAD`）已 fetch 成功却不再含该路径，即为 `removed`；fetch 失败仍是"未知"，保留上次状态。`removed` 携带后继判定 `successor`：先用 `git diff -M`（经 Git session）比较旧目录与 tracked ref 上新增的容器目录的 `SKILL.md`，命中 `R` 记录即更名并记录相似度；否则比较 frontmatter `name` 相同的新增目录。`update_available` 语义不变（只表示内容更新），频道托管 Skill 两者都清空。`Skill`、`SkillUpdateState`、patrol 事件都带 `upstream_change`；ghost 检测反向把被指为后继的新技能标为 `renamed_from`。
- 两种上游变动在 UI 里是两种操作，都从卡片状态槽与详情面板进入，结果与从 ghost 卡进入一致：`removed` 无后继时 chip「上游已移除」直接打开既有「来源已不再提供」对话框（转为本地副本 / 彻底移除），走 `resolve_skill_update` 的 removed-source 分支并继续同 checkout 存活项的更新；有后继时 chip「迁移到 X」走 `skillstar-app::skill_migration`：记录旧 Skill 的 Agent 链接与项目部署 → 按后继的精确 `folder_path` 从同一仓库安装（这一步 reset checkout，旧内容随之消失，属预期）→ 把新 Skill 链接到同一批 Agent 与项目 → 卸载旧条目，逐步失败时返回已完成到哪一步而不是整体报错。ghost 卡对被指为后继的新技能显示「由 Y 更名而来」并把安装按钮换成同一迁移。侧栏 My Skills 徽标把上游变动计入“需处理”数；「更新 N 项」CTA 与「可更新」筛选只算 `update_available`，不把不可更新项塞进更新。未安装的 ghost 卡打开详情抽屉时，主按钮同样是「迁移到 X」而非裸安装：synthetic skill 携带 `upstream_change`，migrate 用旧名（`renamed_from`）调用，保证部署随迁。
- 更新事务内判定「来源已删除」仍只允许在 pull 之后：检查期的 `upstream_change` 是只读提示，破坏性选择以 pull 后（或 `resolve_skill_update` 当场按 tracked ref）核对的磁盘事实为准。pull 前无法知道新 tree 是否还包含该 Skill。更新一旦发现受该 checkout 支持的已安装 Skill 在 pull 后不再有内容，必须整体回滚到更新前 revision、sparse 配置与受管内容，再把这些名字作为停止项报告；先删后问会让「转为本地副本」无内容可留。可保留还是只能移除，按回滚完成后的磁盘事实判定，不作预测。
- Hub 链接悬空但受跟踪 revision 仍包含该路径时，属于稀疏 checkout 尚未物化，不是用户需要回答的问题：分类不得拦截更新，由随后的 pull 自行物化；只有受跟踪 revision 也不再包含该路径，才判定为来源已删除。
- 本地分歧与来源已删除不共享一个对话框：一个批次里同时出现两类停止时，前端按类别分两轮询问，每轮的按钮对该轮全部条目都成立，不出现对一半条目无效的操作。
- 副本名在第一次后端调用之前完成校验：非空、可作为 Hub 目录名（与后端 `validate_skill_name` 同规则）、批内互不重复、且不与任何已安装 Skill 同名；任一项不通过就禁用「保留副本并更新」并就地提示。逐项事务无法整体回滚，因此名称冲突必须挡在批处理开始前，不能让批次执行到一半才失败。
- 批量解决期间对话框保持打开并显示已完成项数；某一项失败时保留剩余队列、用户已输入的副本名与失败原因，允许改名后重试。后端若在解决后仍把同一 Skill 列为 blocked，前端停止并报错，不重复发送同一决定。
...
- 所有内置与新建自定义 Agent 默认关闭。Settings 开关是本机 Agent 激活状态的唯一来源；只有用户显式启用的 profile 才是 Skill、Deck、Project 与 MCP 等本机 Agent rail 上的 target。GUI 的单项和批量 Global 部署在后端同样必须拒绝未启用 profile，不得因陈旧 UI/IPC 请求创建 `~/.agent/skills` 类目录；CLI 显式 `--agent` / `--all` 仍是用户的直接授权。关闭后不删除已部署内容；未启用的 profile 不出现在卡片轮播上，即使技能仍链接或 MCP server 仍写入。
...
- MCP 卡片轮播只展示手动启用 profile 与静态能力映射的交集。工具栏筛选同样只列当前启用的 profile。不再用 MCP tool 安装探测决定是否渲染；真实写入失败由执行动作返回。SSH 远端 discovery 属于用户显式连接后的远端目录扫描，不复用本机激活规则。
...
- 新技能（ghost）检测 `skillstar-skills::repo_scanner::detect_new_skills_in_cached_repos` 同时看两处：本地 checkout 按根优先规则能发现的 Skill，以及 tracked ref tree 相对本地 `HEAD` tree 新增、且满足同一容器/深度/遮蔽规则的 `SKILL.md` 目录。后者直接从 Git 对象读 manifest（经 Git session：partial clone 的 lazy fetch 遵守代理、镜像与凭据策略），不物化 checkout，也不推进 `HEAD`——工作树只在安装/更新/扫描时移动，单靠它看不到上游新增。检测只在一次更新检查（fetch，或 API 快路径判定 tip 未前进）之后才新鲜，所以前端在每次 `refresh_skill_updates` 完成后重取 `check_new_repo_skills`，工具栏刷新按钮因此同时刷新 ghost；patrol 在预取之后检测并以 `patrol://new-skills-detected` 推送。仓库根变成 Skill 与 plugin manifest 声明的目录不在远端差集内。
...
- 每个 GitHub 仓库来源行在移除按钮左侧提供「重新安装」入口：重新扫描当前仓库的全深度 Skill 清单，并只将这一仓库发现的全部 Skill 重新安装；执行期间该行的重新安装按钮显示加载状态，不影响其他仓库。该仓库存在未解决的本地分歧时，重装在任何写入前 fail-closed，提示指向更新流程（先保留副本或丢弃修改）而不是要求用户自行解释后端错误。
- 本地 scope 工具栏把「来源」筛选与当前列表数量合成同一 pill：左侧为来源标签与下拉/清除，右侧为 `countText`（层叠图标 + 数量）；无来源筛选时数量仍单独成 pill（远端 scope 等同）。
- 本地 scope 处理待更新的默认路径是独立主 CTA「更新 N 项」（与「待更新」筛选分离）：一点即更新 Hub 内全部已标记 `update_available` 的技能（不受当前筛选影响），名单以点击瞬间快照为准，无确认框；结束用既有汇总 toast。单卡「更新」保留为次要 ghost 入口。决策见 [Wayfinder: 更新全部成为默认更新路径](https://github.com/xxww0098/SkillStar/issues/16)。旁边「可更新 (N)」筛选块的 N 与 CTA 不同源：它算的是当前搜索/来源/仓库筛选之后仍可更新的数量，因此点开筛选一定能看到 N 张卡；筛选把待更新技能挡掉时 N 显示 0，而 CTA 仍按全局出现。筛选激活时列表上方显示一条状态横幅（「已筛选：仅显示有更新的技能（N）」+「显示全部」），列表骤减不会被误读为技能丢失。
- 技能详情抽屉打开时内容区在 ≥1024px 视口让位 `pr-[448px]`（抽屉 `max-w-md`），最右列卡片不再被盖住；更窄窗口保持覆盖式。进入批量选择的一次性快捷键提示 toast 从 `bottom-center` 弹出，不遮抽屉底部主操作按钮。
- 详情抽屉只保留一个当前选择：已安装技能按 Hub 身份读取最新列表数据，ghost 则保留仓库与发现路径，不能仅因同名而替换来源。更新完成只刷新技能数据，不得重新打开用户已关闭的抽屉，也不得覆盖期间切换的选择。ghost 迁移命令和进行中状态共用被迁移的旧名；关闭后重新打开仍须显示同一进行中操作，不能重复提交。
...
- `useSkills` 是「更新 + 解决停止项」的唯一所有者：`runSkillUpdate(names)` 先批量拉取，再把 blocked 交给 `useLocalDivergenceResolver` 驱动到底，返回已合并的最终 report（含被移除的名字）。对话框只有一个实例（挂在 `SkillsProvider` 上），页面不得再各自持有 blocked 队列或重复 `resolve_skill_update` 循环；按类别分轮询问也由该 hook 负责，页面不感知轮次。
- 浏览器 dev 的 `lib/ipc/devMock/skillsUpdateStore.ts` 是可变的更新状态源：更新真的会清除 `update_available`，并按 checkout 返回 blocked、逐项解决与仓库重装的 fail-closed，使这些路径不进 Tauri 也能实测。它同时预置两类停止项（本地分歧与来源已删除，后者含内容尚在与内容已丢失各一），以覆盖分轮对话框和移除出口。
