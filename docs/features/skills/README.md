...
- 界面必须说明凭据写在 `SKILLSTAR_DATA_DIR/state/github_auth.json`（Unix `0600`），**不是**系统钥匙串；不要在登录文案里写「钥匙串 / credential store」。
...
- 所有内置与新建自定义 Agent 默认关闭。Settings 开关是本机 Agent 激活状态的唯一来源；只有用户显式启用的 profile 才是 Skill、Deck、Project 与 MCP 等本机 Agent rail 上的**可操作** target。GUI 的单项和批量 Global 部署在后端同样必须拒绝未启用 profile，不得因陈旧 UI/IPC 请求创建 `~/.agent/skills` 类目录；CLI 显式 `--agent` / `--all` 仍是用户的直接授权。关闭后不删除已部署内容；rail 上若该资源仍挂着这个 Agent（技能已链接 / MCP server 已写入），SVG 进入停用态（灰度、不可点），让启停可被卡片感知，而不是从轮播里消失。未挂上的 Agent 关闭后不占轮播位。
...
- MCP 卡片轮播在手动启用 profile 与静态能力映射的交集上，再并上这条 server 已经写入的 target：Settings 关掉但 `enabled[toolId]` 仍为 true 的 Agent 留在轮播里，SVG 进停用态。工具栏筛选仍只列当前启用的 profile。不再用 MCP tool 安装探测决定是否渲染；真实写入失败由执行动作返回。SSH 远端 discovery 属于用户显式连接后的远端目录扫描，不复用本机激活规则。
