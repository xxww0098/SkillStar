...
- 提交走 `mcp_market_install`：它把答案、启用目标和**用户确认过的那条字符串**一起交给 `prepare_install`（同样是纯函数，包在 `preview_install` 之上，不重新推导），后者重新推导后逐字比对，一致才返回校验通过的 entry；加锁、读 store、`create_server_and_sync` 与投影留在命令适配器。比对目标就是确认步骤里实际看到的那一条：stdio 形态比对命令预览，远端形态比对解析后的 URL。
- 这条比对不是可选的：提交时目录行是**从本地快照重新读取**的，而目录同步可能在用户阅读预览期间改写这一行。比对的是未掩码的原串——掩码只发生在展示边缘（`maskSecrets`），先掩码再比对会让任何带密钥参数的安装永远对不上。
- 拒绝有两种，且必须可区分：`missingInputs` 指出缺的是哪一项，`commandChanged` 说明这已不是你确认过的那条命令。拒绝时什么都没写入，前端保留用户已填的内容并重新拉一次预览（否则屏幕上仍是旧命令，再确认一次还会被拒）。
