...
- 安装向导必须展示完整命令预览与运行时候选；secret 字段必须掩码，且不得回显进日志。用户填写的参数会改变命令行，因此确认步骤显示的是 `mcp_market_install_preview` 按最终值渲染出的那一条——前端不持有任何参数拼装、模板替换或命令行渲染逻辑，只做掩码与即时的必填/格式/可选值校验（纯函数，即时反馈不该走一趟 IPC；后端的校验才是权威）。预览按 300ms 去抖调用，在途期间提交按钮禁用，避免批准一条过期的命令。本地（stdio）安装必须勾选确认框才能提交。
