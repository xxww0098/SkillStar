...
- Tauri command/core 只保留 ACP 进程、State、事件和 DTO 适配。Learn 一级页、Guide 阅读、本地进度和显式 HTML→Draft 转换由本域拥有；社区发布不在 P0。
...
- Guide：`~/.skillstar/learning/guides/`（P0 首发 Guide 内置于 `skillstar-learning::guide::seed`，不依赖安装表）。
- 进度：`~/.skillstar/learning/progress/<guide-id>/<revision-key>.json`，key 必含 Guide revision；旧 revision 只作为 stale 提示，不得静默套用。
- Draft：`~/.skillstar/learning/drafts/`；转换失败不覆盖最后一个可用 Draft，也不改 HTML tutorial。
...
- `create_guide_draft_from_tutorial` / `preview_guide_draft_from_tutorial`（只接受 bound、已校验 artifact；显式触发）
- `list_guide_drafts`

P0 首发 Guide 绑定 marketplace 落地时的 `anthropics/skills` + `skills/frontend-design`，revision pin 为 catalog commit/tree 加上文档化 seed content hash（`skillstar.seed.frontend-design.p0.v1`），不是 live v2 snapshot。阅读条件只看 Guide 内容与 revision，不看 `skill.installed`。进入 `requiresSkill` 步骤时只预览精确 revision 安装，P0 确认安装走既有 `install_skill`，不执行作者命令。

HTML → Guide Draft 必须由用户在教程面板显式触发。允许的 block 类型是 heading/paragraph/list/code/callout 闭集；未知标签、脚本、事件属性、远程资源 fail closed。转换结果可预览、可 diff、可搜索，没有上传/发布按钮。
