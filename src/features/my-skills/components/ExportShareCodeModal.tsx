...
import { Check, Copy, Download, FileText, KeyRound, Link2, Loader2, Package, Sparkles } from "lucide-react";
...
export function ExportShareCodeModal({
  ...
  useEffect(() => {
    ...
    const analyze = async () => {
      ...
      if (namesNeedingSource.length > 0) {
        try {
          const remoteResolved = await tauriInvoke("resolve_skill_sources", {
      ...
      for (const skillName of activeSkills) {
        ...
        try {
          const content = await tauriInvoke("read_skill_file_raw", {
            name: skillName,
          });
          ...
          let fileCount = 1;
          try {
            const files = await tauriInvoke("list_skill_files", {
              name: skillName,
            });
  ...
  const handleSaveBundleFile = async () => {
    try {
      ...
      setLoading(true);
      setBundleExporting(true);
      await tauriInvoke("export_multi_skill_bundle", {
  ...
  return (
    <ModalShell
      open={open}
      onClose={handleClose}
      ariaLabel={group ? t("exportShareCodeModal.titleDeck") : t("exportShareCodeModal.title")}
      panelClassName="max-w-[420px] max-h-[calc(100vh-2rem)]"
      surfaceClassName="max-h-[calc(100vh-2rem)] flex flex-col"
      contentClassName="flex flex-col min-h-0 flex-1 h-full"
    >
      {/* Header */}
      <ModalHeader
        title={group ? t("exportShareCodeModal.titleDeck") : t("exportShareCodeModal.title")}
        onClose={handleClose}
        className="pt-5 pb-1 border-b-0"
      />

      <div className="px-6 pb-3 text-sm text-muted-foreground leading-relaxed">
        {group ? t("exportShareCodeModal.descriptionDeck") : t("exportShareCodeModal.description")}
      </div>
      ...
      <div className="px-6 pb-6 space-y-4 overflow-y-auto min-h-0 flex-1">
        ...
        {!analyzing && !isResultPhase && (
          <>
              ...
              {mode === "bundle" && (
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-blue-400">{t("exportShareCodeModal.bundleLabel")}</span>
                    <span className="text-micro text-muted-foreground ml-auto">
                  ...
                  <div className="flex flex-wrap gap-1.5 max-h-[160px] overflow-y-auto pr-1">
                    {skillStatuses.map((s) => (
                      ...
                      >
                        ...
                        {s.name}
                        {s.fileCount && s.fileCount > 1 && <span className="opacity-50">({s.fileCount})</span>}
                      </span>
                    ))}
                  </div>
                  <p className="text-micro text-muted-foreground mt-2 leading-relaxed">
            ...
            {/* Export button */}
            <Button onClick={handleExport} disabled={loading || skillStatuses.length === 0} className="w-full mt-1">
          ...
          <div className="space-y-4 animate-in fade-in zoom-in-95 pb-4">
            ...
            {code && code.length > 0 && (
              <div className="space-y-2.5">
                ...
                {hasEmbedded && <p className="text-micro text-primary/70">{t("exportShareCodeModal.embeddedInfo")}</p>}
              </div>
            )}
            ...
            {mode === "bundle" && (
              <div className="space-y-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-foreground">{t("exportShareCodeModal.bundleReady")}</span>
                  <span className="text-micro text-muted-foreground ml-auto">
                    {skillStatuses.length} {skillStatuses.length === 1 ? "skill" : "skills"}
                  </span>
                </div>
