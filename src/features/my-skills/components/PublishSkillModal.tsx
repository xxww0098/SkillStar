...
export function PublishSkillModal({ open, onClose, skillName, skillDescription, onPublished }: PublishSkillModalProps) {
  ...
  const checkGhStatus = useCallback(async () => {
    ...
    setError(null);
    try {
      const status = await tauriInvoke("check_gh_status");
      setGhStatus(status);
  ...
  const selectRepo = async (repo: UserRepo) => {
    ...
    try {
      const folders = await tauriInvoke("inspect_repo_folders", {
  ...
  useEffect(() => {
    if (phase === "form" && skillName) {
      tauriInvoke("list_skill_files", { name: skillName })
        .then(setSkillFiles)
  ...
  const handlePublish = async () => {
    ...
    setError(null);
    try {
      const publishResult = await tauriInvoke("publish_skill_to_github", {
        skillName,
  ...
  return (
    <ModalShell
      open={open}
      onClose={handleClose}
      ariaLabel={t("publishModal.title")}
      panelClassName={`transition duration-300 ease-in-out flex flex-col ${
        phase === "form" ? "max-w-[760px] h-[580px]" : "max-w-md h-auto"
      }`}
      surfaceClassName="flex-1 flex flex-col min-h-0"
      contentClassName="flex flex-col flex-1 h-full min-h-0"
    >
      {/* Header */}
      ...
      <div className="px-6 pb-2 pt-1 text-sm text-muted-foreground leading-relaxed">
        {phase === "pick-repo" ? t("publishModal.pickRepoDesc") : t("publishModal.description")}
      </div>
      ...
      <div className="px-6 py-4 space-y-4 overflow-y-auto flex-1">
        ...
        {phase === "publishing" && (
          <div className="flex flex-col items-center justify-center py-8 gap-3">
            ...
            <p className="text-sm text-muted-foreground">{t("publishModal.publishing", { name: folderName })}</p>
            ...
            <div className="flex items-center gap-3 py-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  <span className="font-medium">{result.source_folder}/</span> →{" "}
                  {result.url.split("/").slice(-2).join("/")}
                </p>
              </div>
            </div>

            <div className="bg-muted rounded-lg px-3 py-2.5 flex items-center gap-2 border border-border/50">
              <code className="text-xs font-mono text-foreground/80 flex-1 truncate select-all">{result.url}</code>
              <button
