...
const AgentRow = memo(function AgentRow({
  ...
  const icon = <AgentIcon profile={profile} className={cn(agentIconCls(profile.icon, "w-5 h-5"), "object-contain")} />;
      ...
      <div className="flex items-center gap-3.5 px-4 py-3.5 sm:px-5">
        {onEdit ? (
          <button
            ...
            className={cn(
              "flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center rounded-xl border bg-card shadow-sm transition-[background-color,border-color,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-muted/60 focus-ring active:translate-y-0",
          ...
          <div
            className={cn(
              "flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border bg-card shadow-sm transition-colors",
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            <span
              className={cn(
                "inline-flex items-center gap-1.5 rounded-md px-1.5 py-0.5 text-[10px] font-medium leading-none",
                readyForCards ? "bg-success/10 text-success" : "bg-muted text-muted-foreground",
              )}
            >
              <span
                aria-hidden="true"
                className={cn("h-1.5 w-1.5 rounded-full", readyForCards ? "bg-success" : "bg-muted-foreground/45")}
              />
          ...
          <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-1.5">
            {displayPaths(profile, platform).map((path) => (
              <span
                key={`${profile.id}-${path}`}
                className="max-w-full break-all rounded-md bg-muted/45 px-1.5 py-0.5 font-mono text-micro text-muted-foreground/70"
              >
                {path}
              </span>
            ))}
                  ...
                  {resolvedLinkedSkillNames.map((skillName) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="cursor-pointer rounded p-1 text-muted-foreground opacity-0 transition group-hover/chip:opacity-100 hover:bg-destructive/10 hover:text-destructive focus-ring focus:opacity-100"
      ...
      <div className="mb-4 flex items-start justify-between gap-4 px-1">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 id="settings-agent-connections-title" className="text-sm font-semibold tracking-tight text-foreground">
        ...
        {onAddCustomProfile && (
          <button
            ...
            className="group flex shrink-0 cursor-pointer items-center gap-1.5 rounded-lg border border-primary/25 bg-primary/10 px-3 py-2 text-xs font-medium text-primary transition-[background-color,border-color,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/40 hover:bg-primary/15 focus-ring active:translate-y-0"
          >
      ...
      {profilesLoading ? (
        ...
        >
          {[0, 1, 2].map((index) => (
            <div
              key={index}
              className="flex animate-pulse items-center gap-3.5 border-b border-border/70 px-5 py-4 last:border-0"
            >
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card/80 shadow-[0_16px_40px_-32px_rgba(15,23,42,0.45)] backdrop-blur-sm">
          <div className="flex items-center justify-between gap-3 border-b border-border/70 bg-muted/20 px-5 py-3">
