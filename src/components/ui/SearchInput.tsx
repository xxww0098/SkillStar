    ...
    return (
      <div className={cn("relative flex-1", containerClassName)}>
        ...
        <Search
          aria-hidden
          className={cn(
            "pointer-events-none absolute left-3 top-1/2 z-[1] -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground/80",
            iconClassName,
          )}
        />
        {suffix && <div className="absolute right-1 top-1/2 z-[1] -translate-y-1/2 flex items-center">{suffix}</div>}
      </div>
    );
  },
);
