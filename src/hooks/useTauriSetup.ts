...
export function useTauriSetup() {
  useEffect(() => {
    ...
    (async () => {
      try {
        // 1. Sync patrol enabled state on mount
        await tauriInvoke("set_patrol_enabled", { enabled: readBackgroundRun() });
      } catch {
      ...
      try {
        ...
        const unlistenHidden = await listen("skillstar://window-hidden", async () => {
          ...
          const status = await tauriInvoke("get_patrol_status").catch(() => null);
          if (status && !status.enabled) {
            await tauriInvoke("set_patrol_enabled", { enabled: true }).catch(() => {});
          }
          await tauriInvoke("start_patrol", {
      ...
      try {
        // 4. Sync language to tray on mount
        await tauriInvoke("update_tray_language", { lang: getLanguage() });
      } catch {
        // Not in Tauri environment
      }
    })();
