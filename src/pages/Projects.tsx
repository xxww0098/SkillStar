...
import type { ProjectDeployMode, ProjectEntry, Skill } from "../types";
...
export function Projects({ preSelectedSkills, onClearPreSelected }: ProjectsProps) {
  ...
  const { handleSelectProject: runProjectSelection } = useProjectSelection({
    enabledProfiles,
    enabledProfileIdSet,
    pathByAgentId,
    canonicalizeAgentsBySharedPath,
  ...
  const handleSelectProject = useCallback(
    async (project: ProjectEntry) => {
      // Reset scan state
      setDetailSkillName(null);
      await runProjectSelection(project);
    },
    [runProjectSelection],
  );
