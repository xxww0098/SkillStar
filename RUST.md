...
- 默认：`artifact=lib, maturity=production`（12 个 `crates/skillstar-*` 成员均只交付 lib target）。
...
- Workspace：虚拟根，13 个成员；`default-members = ["src-tauri"]`。所有成员均为 edition 2024、MSRV 1.94.1、resolver 3；日常构建工具链固定为 1.97.1。
- 规范：rust-skills v0.0.11（112 条分级规则）。
- Features：13 个 package 均未声明 package feature；依赖 feature 由根 `[workspace.dependencies]` 与成员清单选择。
- Lints：13 个成员均继承 workspace lint；`clippy::todo`、`unimplemented`、`dbg_macro` 为 deny，其他存量问题使用项目门禁/基线收紧。
...
消费者到全部内部依赖（`→`；38 条 normal、1 条 dev-only、0 条 build）：

- `skillstar → {app, channels, sync, skills, models, marketplace, usage, agents, github-auth, git, core}`
- `skillstar-app → {channels, skills, models, marketplace, usage, agents, git, core}`
- `skillstar-channels → {skills, github-auth, git, core}`，另有 `skillstar-channels -[dev]→ skillstar-agents`；`skillstar-sync → {skills, core}`
- `skillstar-skills → {github-auth, agents, git, core}`；`skillstar-github-auth → {git, core}`
- `skillstar-models → {providers, core}`；`skillstar-usage → {providers, core}`
- `skillstar-marketplace → core`；`skillstar-agents → core`；`skillstar-git → core`
- 内部依赖出度 0：`skillstar-core`、`skillstar-providers`（仅为 WS-12 候选素材；两者当前均有多个消费者，不构成删除结论）。

最长链（叶子在左，6 条边）：

`skillstar-core ← skillstar-git ← skillstar-github-auth ← skillstar-skills ← skillstar-channels ← skillstar-app ← skillstar`
...
- 业务域：`skillstar-skills`（技能/项目/部署）、`skillstar-channels`（共享频道/patrol）、`skillstar-marketplace`（市场/MCP catalog）、`skillstar-models`（provider/AI/MCP/tool sync）、`skillstar-usage`（订阅/OAuth/配额）、`skillstar-sync`（SSH/SFTP）。
- 叶子能力：`skillstar-git`（Git transport/ops）、`skillstar-agents`（Agent registry/profile）、`skillstar-github-auth`（设备授权/凭据）、`skillstar-providers`（零依赖 provider 元数据）。完整所有权与依赖红线的 SSOT 是 `docs/boundaries.md`，运行和数据所有权的 SSOT 是 `docs/architecture.md`。
