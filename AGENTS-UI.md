...
- Hub layout (top-down):
  ...
  2. **Agent cards section** ("AGENT 接入") — the ONLY place to activate/deactivate/re-sync agents. The card layout is chosen by the agent's `kind` (`lib/agentRegistry.ts`, see [agent kinds](#agent-kinds) below): **single-provider** agents (`claude-code`, `gemini`) render `AgentHeroCard` (one provider picker + one model picker + tier mapping); **multi-provider** agents (`codex`, `opencode`) render `MultiProviderCard`, which lists EVERY bound provider as a row (active radio + per-row model picker + remove) plus an "添加更多供应商" control. Claude Code CLI and Desktop Code are one `claude-code` card and one binding; `AppAiCard` (in-app AI binding) joins the provider-agent cards. The section header shows the aggregate "x/y 已接入" summary — there is no separate HealthBar strip; connection probes live in `hooks/useAgentHealth.ts` (probe the active binding entry once per `(toolId, providerId)` pair, click the status pill to retest) and statuses come from `lib/agentStatus.ts` (the canonical status model; latency colors via `lib/latencyColor.ts`).
  3. **Provider gallery**: search input + responsive `ProviderGalleryCard` grid with hover menu (duplicate / delete-with-confirmation).
- <a id="agent-kinds"></a>**Agent kinds — single vs multi provider.** Agents differ in how they load providers, and the UI reflects this rather than forcing one shape. `AgentDescriptor.kind` (`"single" | "multi"`) is the single switch; `agentSupportsMultipleProviders(toolId)` is its helper. The backend binding is `ToolBinding { entries, active_index }` (mirrored in `types/models.ts`), and `lib/toolBinding.ts` holds the pure helpers (`activeEntry`, `upsertBindingEntry`, `removeBindingEntry`, `setActiveProvider`) so no component indexes `entries[active_index]` raw. `hooks/useAgentActivation.ts` exposes both surfaces from one binding: `activeEntry` (single-provider) and `entries` (multi-provider, each paired with its resolved provider), plus `activate` / `setActive` / `addProvider` / `removeEntry` / `pickModel(model, providerId?)`. Multi-provider mutations (`set_active_binding`, `remove_binding_entry`) live in `api/activations.ts` with optimistic binding updates.
- **Agent settings dialog** (`agents/AgentSettingsDialog.tsx`, ModalShell 640px): per-agent deep configuration — provider/model binding (the active entry), agent-conditional params (`ClaudeModelMapping` for claude-code; `CodexSettingsForm` for codex — applied to the active entry), Claude launch command (`lib/launchCommand.ts`), on-disk config file editor (`AgentConfigFiles`, single-tool mode), conflict warnings (tool-filtered), last-sync time + re-sync, deactivate. Day-to-day multi-provider list management (add / switch active / remove) lives on `MultiProviderCard`; the dialog is for deep params. Activation flows through `hooks/useAgentActivation.ts` — the single activation path shared with the cards.
- **`ProviderEditorDrawer`** (`provider/ProviderEditorDrawer.tsx`, `components/shared/DrawerShell` `max-w-[640px]`): edit mode is a TABBED form — 连接 (name / API key / dual base URLs / models_url) · 模型 (fetch + model list management + default model) · 高级 (runtime params / notes) · 诊断 (`ConnectionStatusPanel` + `EndpointSpeedPanel` + disk config). The drawer owns the autosave state machine: `hooks/useProviderForm.ts` (one reducer-managed values object; pure conversions in `lib/providerPatch.ts`) + `hooks/useAutosave.ts` (600ms debounce, validation-aware re-arm, **flush-on-close** so pending edits are never silently dropped). The save badge (`shared/SaveBadge`) renders ONLY here — the hub no longer mirrors save state.
- Create mode renders `PresetPicker` (category-tiled preset grid → API key + base URL → 创建并继续) inside a plain `components/shared/DrawerShell`, then pivots to the editor drawer with the created provider showing a one-time `PostCreateGuide` banner (添加供应商 → 测试连接 → 接入 Agent step trail; auto-fetches the model catalog once when credentials allow). Provider deletion always confirms via `DeleteProviderDialog`, listing the agents that would disconnect.
...
- **`ScopeDetailDrawer`** (`my-skills/components/ScopeDetailDrawer.tsx`) is the single detail entry point: a discriminated union (`kind: 'local' | 'remote'`) giving compile-time capability enforcement. The `local` arm renders the lazy `DetailPanel` (unchanged, absolute-positioned inside the page's `relative` container); the `remote` arm renders `RemoteSkillDrawer`, which now composes `components/shared/DrawerShell` (canonical Radix scrim/focus/animation) instead of a hand-rolled sheet.
...
- Follow-up (out of scope here): converging `DetailPanel` (682 lines) onto `components/shared/DrawerShell` — left for a dedicated pass per the file-size rule.
...
- Feature slices (`src/features/<domain>/`) own their internal components/hooks/api; a component inside one feature must not be imported by another feature. Purely presentational components with no feature business logic that need cross-feature reuse (e.g. `DrawerShell`, `ProviderBrandIcon`) belong in `src/components/shared/` instead. This boundary is enforced by a CI gate (`scripts/internal/check_feature_imports.sh`) with a shrink-only grandfathered baseline (`feature_imports_baseline.txt`) — new cross-feature imports fail the build; intentional, already-existing ones must be explicitly baselined.
