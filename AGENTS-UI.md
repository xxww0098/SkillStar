...
- Hub layout (top-down):
  ...
  2. **Agent cards section** ("AGENT 接入") — the ONLY place to activate/deactivate/re-sync agents. The card layout is chosen by the agent's `kind` (`lib/agentRegistry.ts`, see [agent kinds](#agent-kinds) below): **single-provider** agents (`claude-code`, `gemini`) render `AgentHeroCard` (one provider picker + one model picker + tier mapping); **multi-provider** agents (`codex`, `opencode`) render `MultiProviderCard`, which lists EVERY bound provider as a row (active radio + per-row model picker + remove) plus an "添加更多供应商" control. Both are joined by `ClaudeDesktopCard` (MCP config, no provider binding) and `AppAiCard` (in-app AI binding). The section header shows the aggregate "x/y 已接入" summary — there is no separate HealthBar strip; connection probes live in `hooks/useAgentHealth.ts` (probe the active binding entry once per `(toolId, providerId)` pair, click the status pill to retest) and statuses come from `lib/agentStatus.ts` (the canonical status model; latency colors via `lib/latencyColor.ts`).
