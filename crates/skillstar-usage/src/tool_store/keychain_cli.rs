...
pub(crate) fn find_internet_password(server: &str) -> UsageResult<Option<InternetPassword>> {
    ...
    let password_output = spawn_security(&["find-internet-password", "-s", server, "-w"])?;
...
mod tests {
    ...
    fn sandboxed_internet_password_ops_do_not_spawn_security() {
        ...
        for error in [
            find_internet_password("https://zed.dev").expect_err("find"),
