...
use skillstar_ai::ai_provider::{AiConfig, chat_completion, chat_completion_capped};
...
use crate::policy::{get_policy, resolve_enabled_analyzers, resolve_policy, save_policy};
use crate::smart_rules;
use crate::static_patterns::static_pattern_scan;
use crate::types::{
    ...
    StaticFinding, clamp_confidence, default_ai_finding_confidence,
    default_confidence_for_severity, default_confidence_score, default_static_finding_confidence,
    parse_confidence_from_json,
};

// ── Logging ─────────────────────────────────────────────────────────

fn log_path() -> PathBuf {
    skillstar_infra::paths::security_scan_log_path()
}

pub fn scan_logs_dir() -> PathBuf {
    skillstar_infra::paths::security_scan_logs_dir()
}
...
fn build_evidence_trail(
    static_findings: &[StaticFinding],
    ai_findings: &[AiFinding],
    analyzer_executions: &[AnalyzerExecutionSummary],
) -> Vec<EvidenceTrailEntry> {
    let mut trail =
        Vec::with_capacity(static_findings.len() + ai_findings.len() + analyzer_executions.len());

    let mut ordered_static = static_findings.to_vec();
    ordered_static.sort_by(|a, b| {
        a.file_path
            .cmp(&b.file_path)
    ...
    for finding in ordered_static {
        let mut excerpts = Vec::new();
        if !finding.snippet.trim().is_empty() {
            excerpts.push(finding.snippet.clone());
        }
        if !finding.description.trim().is_empty() {
            excerpts.push(finding.description.clone());
        }
        trail.push(EvidenceTrailEntry {
            kind: EvidenceTrailKind::StaticFinding,
            file_path: Some(finding.file_path),
            line_number: Some(finding.line_number),
            source_id: Some(finding.pattern_id),
            severity: Some(finding.severity),
            confidence: Some(finding.confidence),
            summary: Some(finding.description),
            excerpts,
    ...
    let mut ordered_ai = ai_findings.to_vec();
    ordered_ai.sort_by(|a, b| {
        a.file_path
            .cmp(&b.file_path)
            .then(a.category.cmp(&b.category))
            .then(a.description.cmp(&b.description))
    });
    for finding in ordered_ai {
        let mut excerpts = Vec::new();
        if !finding.evidence.trim().is_empty() {
            excerpts.push(finding.evidence.clone());
        }
        if !finding.recommendation.trim().is_empty() {
            excerpts.push(finding.recommendation.clone());
        }
        trail.push(EvidenceTrailEntry {
            kind: EvidenceTrailKind::AiFinding,
            file_path: Some(finding.file_path),
            line_number: None,
            source_id: Some(finding.category),
            severity: Some(finding.severity),
            confidence: Some(finding.confidence),
            summary: Some(finding.description),
            excerpts,
    ...
    let mut ordered_analyzers = analyzer_executions.to_vec();
    ordered_analyzers.sort_by(|a, b| a.id.cmp(&b.id).then(a.status.cmp(&b.status)));
    for execution in ordered_analyzers {
        let analyzer_id = execution.id.clone();
        let analyzer_status = execution.status.clone();
        let analyzer_findings = execution.findings;
        let mut excerpts = vec![format!("status={}", execution.status)];
        if let Some(error) = execution
            .error
            .clone()
            .filter(|value| !value.trim().is_empty())
        {
            excerpts.push(error.clone());
            trail.push(EvidenceTrailEntry {
                kind: EvidenceTrailKind::AnalyzerSummary,
                file_path: None,
                line_number: None,
                source_id: Some(analyzer_id.clone()),
                severity: None,
                confidence: None,
                summary: Some(format!(
                    "Analyzer {} completed with status {} and {} finding(s)",
                    analyzer_id, analyzer_status, analyzer_findings
                )),
                excerpts,
                taxonomy: None,
                analyzer_status: Some(analyzer_status),
                analyzer_findings: Some(analyzer_findings),
                analyzer_error: Some(error),
            });
        } else {
            trail.push(EvidenceTrailEntry {
                kind: EvidenceTrailKind::AnalyzerSummary,
...
fn parse_usize_loose(value: &str) -> Option<usize> {
    value.trim().parse().ok()
}
...
fn parse_f32_loose(value: &str) -> Option<f32> {
    value.trim().parse().ok()
}
...
fn trim_list_item_prefix(line: &str) -> &str {
    line.trim().strip_prefix("- ").unwrap_or(line.trim())
}

fn parse_scan_error_line(line: &str) -> Option<SecurityScanAuditError> {
    let content = trim_list_item_prefix(line);
    ...
    let message = content
        ...
        .trim_matches('"')
        .to_string();
    Some(SecurityScanAuditError {
        skill_name,
        message,
    })
}

fn parse_risk_with_confidence(token: &str) -> (Option<RiskLevel>, Option<f32>) {
    let inner = token.trim().trim_start_matches('[').trim_end_matches(']');
    let mut parts = inner.split('|');
    let risk_level = parts.next().map(RiskLevel::from_str_loose);
    let confidence = parts
        .find_map(|part| part.trim().strip_prefix("conf="))
        .and_then(parse_f32_loose);
    (risk_level, confidence)
}

fn parse_static_audit_finding(line: &str) -> Option<SecurityScanAuditFinding> {
    let content = trim_list_item_prefix(line);
    let (risk_token, remainder) = content.split_once("] ")?;
    let (risk_level, confidence) = parse_risk_with_confidence(risk_token);
    let (prefix, label) = remainder.rsplit_once(" (")?;
    let label = label.trim_end_matches(')').trim().to_string();
    let (path_and_line, description) = prefix.split_once(' ')?;
    let (file_path, line_number) = path_and_line.rsplit_once(':').and_then(|(path, line)| {
        parse_usize_loose(line).map(|line_number| (path.to_string(), line_number))
    })?;

    Some(SecurityScanAuditFinding {
...
fn parse_ai_audit_finding(line: &str) -> Option<SecurityScanAuditFinding> {
    let content = trim_list_item_prefix(line);
    let (risk_token, remainder) = content.split_once("] ")?;
    let (risk_level, confidence) = parse_risk_with_confidence(risk_token);
    let (file_path, category_and_desc) = remainder.split_once(' ')?;
    let (label, description) = category_and_desc.split_once(": ")?;

    Some(SecurityScanAuditFinding {
        finding_type: "ai".to_string(),
        risk_level,
        confidence,
...
fn parse_meta_counts(value: &str) -> (usize, usize) {
    let mut deduped = 0;
    let mut consensus = 0;
    for part in value.split_whitespace() {
        if let Some(raw) = part.strip_prefix("deduped=") {
            deduped = parse_usize_loose(raw).unwrap_or(0);
        }
        if let Some(raw) = part.strip_prefix("consensus=") {
            consensus = parse_usize_loose(raw).unwrap_or(0);
        }
    }
    (deduped, consensus)
}

fn parse_file_metrics(value: &str) -> (Option<usize>, Option<usize>) {
    let mut files = None;
    let mut chars = None;
    for part in value.split_whitespace() {
        if let Some(raw) = part.strip_prefix("files_scanned:") {
            files = parse_usize_loose(raw);
        } else if let Some(raw) = part.strip_prefix("total_chars:") {
            chars = parse_usize_loose(raw);
        }
    }
    if files.is_none() || chars.is_none() {
        let tokens = value.split_whitespace().collect::<Vec<_>>();
        let mut idx = 0;
        while idx + 1 < tokens.len() {
            match tokens[idx] {
                "files_scanned:" => files = parse_usize_loose(tokens[idx + 1]),
                "total_chars:" => chars = parse_usize_loose(tokens[idx + 1]),
                _ => {}
            }
            idx += 1;
        }
...
fn parse_findings_counts(value: &str) -> (usize, usize) {
    let mut static_count = 0;
    let mut ai_count = 0;
    for part in value.split_whitespace() {
        if let Some(raw) = part.strip_prefix("static=") {
            static_count = parse_usize_loose(raw).unwrap_or(0);
        }
        if let Some(raw) = part.strip_prefix("ai=") {
            ai_count = parse_usize_loose(raw).unwrap_or(0);
        }
    }
    (static_count, ai_count)
}

fn parse_risk_score_line(value: &str) -> (Option<f32>, Option<f32>) {
    let score = value
        .split_once("/10")
        .and_then(|(prefix, _)| prefix.split_whitespace().last())
        .and_then(parse_f32_loose);
    let confidence = value
        .split_once("(confidence ")
        .and_then(|(_, rest)| rest.strip_suffix(')'))
        .and_then(parse_f32_loose);
    (score, confidence)
}

fn telemetry_for_created_at(created_at: &str) -> Option<SecurityScanAuditTelemetrySnapshot> {
    let path = scan_telemetry_path();
    let raw = std::fs::read_to_string(path).ok()?;
    raw.lines()
        .filter(|line| !line.trim().is_empty())
        .filter_map(|line| serde_json::from_str::<SecurityScanTelemetryEntry>(line).ok())
        .rev()
        .find(|entry| entry.recorded_at.as_str() <= created_at)
        .map(|entry| SecurityScanAuditTelemetrySnapshot {
            ...
            targets_total: entry.targets_total,
            results_total: entry.results_total,
            pass_count: entry.pass_count,
            pass_rate: entry.pass_rate,
            incomplete_count: entry.incomplete_count,
            error_count: entry.error_count,
            risk_distribution: entry.risk_distribution,
        })
}

fn parse_scan_log_detail(entry: &SecurityScanLogEntry) -> SecurityScanAuditDetail {
    let raw = match std::fs::read_to_string(&entry.path) {
        Ok(raw) => raw,
        Err(err) => {
            return SecurityScanAuditDetail {
                ...
                parse_warnings: vec![format!("failed to read log: {err}")],
                ..Default::default()
            };
        }
    };
    ...
    let mut summary = SecurityScanAuditSummary {
        file_name: entry.file_name.clone(),
        path: entry.path.clone(),
        created_at: entry.created_at.clone(),
        size_bytes: entry.size_bytes,
        telemetry: telemetry_for_created_at(&entry.created_at),
        ..Default::default()
    };
    let mut cached_skills = Vec::new();
    let mut errors = Vec::new();
    let mut skills = Vec::new();
    let mut parse_warnings = Vec::new();
    let mut current_skill: Option<SecurityScanAuditSkillDetail> = None;
    let mut section = Section::None;

    let finalize_current_skill =
        |current_skill: &mut Option<SecurityScanAuditSkillDetail>,
         skills: &mut Vec<SecurityScanAuditSkillDetail>,
         summary: &mut SecurityScanAuditSummary| {
            if let Some(skill) = current_skill.take() {
                if let Some(risk) = skill.risk_level {
                    summary.highest_risk = Some(match summary.highest_risk {
                        Some(existing) => RiskLevel::max(existing, risk),
                        None => risk,
                    });
                }
                if skill.incomplete == Some(true) {
    ...
    for raw_line in raw.lines() {
        ...
        if trimmed.is_empty() {
            if matches!(section, Section::StaticFindings | Section::AiFindings) {
                section = Section::None;
            }
            continue;
        }
        if trimmed == "SkillStar Security Scan Report"
        ...
        if let Some(rest) = trimmed.strip_prefix("Skill: ") {
            finalize_current_skill(&mut current_skill, &mut skills, &mut summary);
            current_skill = Some(SecurityScanAuditSkillDetail {
        ...
        match trimmed {
            ...
            "static_findings:" => {
                section = Section::StaticFindings;
                continue;
            }
            "ai_findings:" => {
                section = Section::AiFindings;
                continue;
            }
            _ => {}
        }
        ...
        match section {
            Section::CachedSkills if trimmed.starts_with("- ") => {
                cached_skills.push(trim_list_item_prefix(trimmed).to_string());
                continue;
            }
            Section::Errors if trimmed.starts_with("- ") => {
                if let Some(error) = parse_scan_error_line(trimmed) {
                    errors.push(error);
                } else {
                    parse_warnings.push(format!("unparsed error line: {trimmed}"));
                }
                continue;
            }
            Section::StaticFindings if trimmed.starts_with("- ") => {
                if let Some(skill) = current_skill.as_mut() {
                    if let Some(finding) = parse_static_audit_finding(trimmed) {
                        skill.findings.push(finding);
                    } else {
                        parse_warnings.push(format!("unparsed static finding line: {trimmed}"));
                    }
                }
                continue;
            }
            Section::AiFindings if trimmed.starts_with("- ") => {
                if let Some(skill) = current_skill.as_mut() {
                    if let Some(finding) = parse_ai_audit_finding(trimmed) {
                        skill.findings.push(finding);
                    } else {
                        parse_warnings.push(format!("unparsed ai finding line: {trimmed}"));
                    }
                }
                continue;
            }
            Section::AiFindings => {
                if let Some(skill) = current_skill.as_mut() {
                    if let Some(last) = skill.findings.last_mut() {
                        if let Some(value) = trimmed.strip_prefix("evidence: ") {
                            last.evidence = Some(value.trim().to_string());
                            continue;
                        }
                        if let Some(value) = trimmed.strip_prefix("recommendation: ") {
                            last.recommendation = Some(value.trim().to_string());
                            continue;
                        }
                    }
                }
        ...
        if current_skill.is_none() {
            if let Some((key, value)) = parse_key_value_line(trimmed) {
                match key {
                    "request_id" => summary.request_id = Some(value.to_string()),
                    "requested_mode" => summary.requested_mode = Some(value.to_string()),
                    "effective_mode" => summary.effective_mode = Some(value.to_string()),
                    "force" => summary.force = parse_bool_loose(value),
                    "started_at" => summary.started_at = Some(value.to_string()),
                    "finished_at" => summary.finished_at = Some(value.to_string()),
                    "duration_ms" => summary.duration_ms = parse_i64_loose(value),
                    "targets_total" => summary.targets_total = parse_usize_loose(value),
                    "cached_hits" => summary.cached_hits = parse_usize_loose(value).unwrap_or(0),
                    "completed_results" => {
                        summary.completed_results = parse_usize_loose(value).unwrap_or(0)
                    }
                    "errors" => summary.error_count = parse_usize_loose(value).unwrap_or(0),
                    _ => {}
                }
            }
            continue;
        ...
        if let Some(skill) = current_skill.as_mut() {
            if let Some((key, value)) = parse_key_value_line(trimmed) {
                match key {
                    "scanned_at" => skill.scanned_at = Some(value.to_string()),
                    "risk" => skill.risk_level = Some(RiskLevel::from_str_loose(value)),
                    "risk_score" => {
                        let (risk_score, confidence_score) = parse_risk_score_line(value);
                        skill.risk_score = risk_score;
                        skill.confidence_score = confidence_score;
                    }
                    "meta" => {
                        let (deduped, consensus) = parse_meta_counts(value);
                        skill.meta_deduped_count = deduped;
                        skill.meta_consensus_count = consensus;
                    }
                    "scan_mode" => skill.scan_mode = Some(value.to_string()),
                    "scanner_version" => skill.scanner_version = Some(value.to_string()),
                    "incomplete" => skill.incomplete = parse_bool_loose(value),
                    "files_scanned" => {
                        let (files_scanned, total_chars_analyzed) = parse_file_metrics(value);
                        skill.files_scanned = files_scanned;
                        skill.total_chars_analyzed = total_chars_analyzed;
                    }
                    "findings" => {
                        let (static_count, ai_count) = parse_findings_counts(value);
                        skill.static_findings_count = static_count;
                        skill.ai_findings_count = ai_count;
                    }
                    "tree_hash" => skill.tree_hash = Some(value.to_string()),
                    "summary" => skill.summary = Some(value.to_string()),
                    _ => {}
                }
            }
        }
    ...
    summary.request_hash = summary
        .telemetry
        .as_ref()
        .map(|telemetry| telemetry.request_hash.clone());
    summary.skill_count = skills.len();
    if summary.completed_results == 0 {
        summary.completed_results = skills.len();
    }
    if summary.error_count == 0 {
        summary.error_count = errors.len();
    }
    summary.parse_warnings = parse_warnings.len();
...
pub fn list_scan_audit_summaries(limit: usize) -> Vec<SecurityScanAuditSummary> {
    list_scan_log_entries(limit)
        .into_iter()
        .map(|entry| parse_scan_log_detail(&entry).summary)
        .collect()
}

pub fn get_scan_audit_detail(file_name: &str) -> Option<SecurityScanAuditDetail> {
    let target = file_name.trim();
    if target.is_empty() {
        return None;
    }
    list_scan_log_entries(200)
        .into_iter()
        .find(|entry| entry.file_name == target)
        .map(|entry| parse_scan_log_detail(&entry))
}
...
fn html_badge(label: &str, value: impl AsRef<str>) -> String {
    format!(
        "<span class=\"badge\"><span class=\"badge-label\">{}</span><span class=\"badge-value\">{}</span></span>",
        escape_html(label),
        escape_html(value.as_ref())
    )
}

fn html_metric_card(label: &str, value: impl AsRef<str>) -> String {
    format!(
...
fn html_empty_state(message: &str) -> String {
    format!("<div class=\"empty\">{}</div>", escape_html(message))
}

fn html_optional_paragraph(class_name: &str, label: &str, value: &str) -> String {
...
fn html_taxonomy_summary(taxonomy: Option<&crate::types::DetectionTaxonomy>) -> Option<String> {
    ...
    if let Some(kind) = &taxonomy.detection_kind {
        parts.push(format!("{} / {}", kind.family.label(), kind.kind));
    }
    if !taxonomy.tags.is_empty() {
        parts.push(format!("tags: {}", taxonomy.tags.join(", ")));
    }
...
fn render_risk_distribution(results: &[SecurityScanResult]) -> String {
    let mut tally = RiskTally::default();
    for result in results {
        tally.add(result.risk_level);
    }
    ...
    .into_iter()
    .map(|(label, count)| html_metric_card(label, count.to_string()))
    .collect::<Vec<_>>()
    .join("")
}

fn render_report_summary(results: &[SecurityScanResult]) -> String {
    let total_findings = results
        .iter()
        .map(|result| result.static_findings.len() + result.ai_findings.len())
        .sum::<usize>();
    let incomplete = results.iter().filter(|result| result.incomplete).count();
    let static_findings = results
        .iter()
        .map(|result| result.static_findings.len())
        .sum::<usize>();
    let ai_findings = results
        .iter()
        .map(|result| result.ai_findings.len())
        .sum::<usize>();
    let analyzer_runs = results
        .iter()
        .map(|result| result.analyzer_executions.len())
        .sum::<usize>();
    let evidence_entries = results
        .iter()
        .map(|result| result.evidence_trail.len())
        .sum::<usize>();
    let avg_score = if results.is_empty() {
        0.0
    } else {
        results.iter().map(|result| result.risk_score).sum::<f32>() / results.len() as f32
    };
    let primary_metrics = html_metric_card("Skills", results.len().to_string())
        + &html_metric_card("Total Findings", total_findings.to_string())
        + &html_metric_card("Average Risk Score", format!("{avg_score:.1}/10"))
        + &html_metric_card("Incomplete Results", incomplete.to_string());
    let secondary_metrics = html_metric_card("Static Findings", static_findings.to_string())
        + &html_metric_card("AI Findings", ai_findings.to_string())
        + &html_metric_card("Analyzer Runs", analyzer_runs.to_string())
        + &html_metric_card("Evidence Entries", evidence_entries.to_string());

    format!(
        "<section class=\"report-section summary-section\"><div class=\"section-heading\"><h2>Report Summary</h2><p>Generated {} · Scanner {}</p></div><div class=\"metrics-grid\">{}</div><div class=\"metrics-grid metrics-grid-compact\">{}</div><div class=\"risk-distribution\"><h3>Risk Distribution</h3><div class=\"metrics-grid metrics-grid-compact\">{}</div></div></section>",
        chrono::Local::now().format("%Y-%m-%d %H:%M:%S"),
...
fn render_skill_overview(result: &SecurityScanResult) -> String {
    let finding_count = result.static_findings.len() + result.ai_findings.len();
    ...
    format!(
        "<section class=\"skill-overview\"><div class=\"section-heading\"><h3>Skill Summary</h3><p>{}</p></div><div class=\"metrics-grid metrics-grid-compact\">{}{}</div><p class=\"summary\">{}</p></section>",
        escape_html(&result.scanned_at),
        html_metric_card("Risk", risk_label(result.risk_level)),
        html_metric_card("Risk Score", format!("{:.1}/10", result.risk_score))
            + &html_metric_card(
                "Confidence",
                format!("{:.0}%", result.confidence_score * 100.0)
            )
            + &html_metric_card("Findings", finding_count.to_string())
            + &html_metric_card("Files", result.files_scanned.to_string())
            + &html_metric_card("AI Files", result.ai_files_analyzed.to_string())
            + &html_metric_card("Chars", result.total_chars_analyzed.to_string())
            + &html_metric_card("Mode", &result.scan_mode)
            + &html_metric_card("Incomplete", result.incomplete.to_string())
            + &html_metric_card("Tree Hash", tree_hash),
        escape_html(&result.summary)
    )
}

fn render_analyzer_executions(analyzers: &[AnalyzerExecutionSummary]) -> String {
    if analyzers.is_empty() {
        return html_empty_state("No analyzer execution metadata recorded.");
    }

    let items = analyzers
        .iter()
        .map(|exec| {
            let status_class = format!("status-{}", exec.status.to_ascii_lowercase());
            let error_block = exec
                .error
                .as_deref()
                .filter(|value| !value.trim().is_empty())
                .map(|value| {
                    format!(
                        "<p class=\"analyzer-error\"><strong>Error:</strong> {}</p>",
                        escape_html(value)
                    )
                })
                .unwrap_or_default();
            format!(
                "<article class=\"analyzer-card\"><div class=\"analyzer-header\"><h4>{}</h4><span class=\"status-pill {}\">{}</span></div><div class=\"badge-row\">{}{}</div>{}</article>",
                escape_html(&exec.id),
                escape_html(&status_class),
                escape_html(&exec.status),
                html_badge("Findings", exec.findings.to_string()),
                html_badge("Recorded", if exec.error.is_some() { "with error" } else { "clean" }),
                error_block
            )
        })
        .collect::<Vec<_>>()
    ...
    format!("<div class=\"stack\">{items}</div>")
}

fn render_static_findings(findings: &[StaticFinding]) -> String {
    if findings.is_empty() {
        return html_empty_state("No static findings.");
    }
    ...
    let items = findings
        ...
        .map(|finding| {
            ...
            let taxonomy = html_taxonomy_summary(finding.taxonomy.as_ref())
                .map(|value| format!("<p class=\"taxonomy\"><strong>Taxonomy:</strong> {}</p>", escape_html(&value)))
                .unwrap_or_default();
            format!(
                "<article class=\"finding-card\"><div class=\"finding-header\"><h4>{}</h4><div class=\"badge-row\">{}{}</div></div><p class=\"finding-location\">{}:{}</p><p class=\"finding-description\">{}</p><pre class=\"code-block\">{}</pre><p class=\"owasp\"><strong>OWASP:</strong> {}</p>{}</article>",
                escape_html(&finding.pattern_id),
                html_badge("Severity", risk_label(finding.severity)),
                html_badge("Confidence", format!("{:.0}%", finding.confidence * 100.0)),
                escape_html(&finding.file_path),
                finding.line_number,
                escape_html(&finding.description),
                escape_html(&finding.snippet),
                escape_html(&owasp),
                taxonomy
            )
        })
...
fn render_ai_findings(findings: &[AiFinding]) -> String {
    if findings.is_empty() {
        return html_empty_state("No AI findings.");
    }
    ...
    let items = findings
        ...
        .map(|finding| {
            ...
            let taxonomy = html_taxonomy_summary(finding.taxonomy.as_ref())
                ...
                .unwrap_or_default();
            format!(
                "<article class=\"finding-card\"><div class=\"finding-header\"><h4>{}</h4><div class=\"badge-row\">{}{}</div></div><p class=\"finding-location\">{}</p><p class=\"finding-description\">{}</p>{}{}<p class=\"owasp\"><strong>OWASP:</strong> {}</p>{}</article>",
                escape_html(&finding.category),
                html_badge("Severity", risk_label(finding.severity)),
                html_badge("Confidence", format!("{:.0}%", finding.confidence * 100.0)),
                escape_html(&finding.file_path),
                escape_html(&finding.description),
                html_optional_paragraph("finding-evidence", "Evidence:", &finding.evidence),
                html_optional_paragraph("finding-recommendation", "Recommendation:", &finding.recommendation),
                escape_html(&owasp),
                taxonomy
            )
        })
...
fn render_evidence_trail(entries: &[EvidenceTrailEntry]) -> String {
    if entries.is_empty() {
        return html_empty_state("No evidence trail captured.");
    }
    ...
    let items = entries
        ...
        .map(|entry| {
            let kind = match entry.kind {
                EvidenceTrailKind::StaticFinding => "static finding",
                EvidenceTrailKind::AiFinding => "ai finding",
                EvidenceTrailKind::AnalyzerSummary => "analyzer summary",
            };
            let location = entry
                .file_path
                .as_deref()
                .map(|path| match entry.line_number {
                    Some(line) => format!("{path}:{line}"),
                    None => path.to_string(),
                })
                .unwrap_or_else(|| "global".to_string());
            let severity_badge = entry
                .severity
                .map(|severity| html_badge("Severity", risk_label(severity)))
                .unwrap_or_default();
            let confidence_badge = entry
                .confidence
                .map(|confidence| html_badge("Confidence", format!("{:.0}%", confidence * 100.0)))
                .unwrap_or_default();
            let analyzer_badge = entry
                .analyzer_status
                .as_deref()
                .map(|status| html_badge("Analyzer Status", status))
                .unwrap_or_default();
            let findings_badge = entry
                .analyzer_findings
                .map(|count| html_badge("Analyzer Findings", count.to_string()))
                .unwrap_or_default();
            let taxonomy = html_taxonomy_summary(entry.taxonomy.as_ref())
                .map(|value| format!("<p class=\"taxonomy\"><strong>Taxonomy:</strong> {}</p>", escape_html(&value)))
                .unwrap_or_default();
            let excerpts = if entry.excerpts.is_empty() {
                String::new()
            } else {
                let blocks = entry
                    .excerpts
                    .iter()
                    .map(|excerpt| format!("<li><pre class=\"code-block inline\">{}</pre></li>", escape_html(excerpt)))
                    .collect::<Vec<_>>()
                    .join("");
                format!("<div class=\"evidence-excerpts\"><strong>Excerpts</strong><ul>{blocks}</ul></div>")
            };
            let error = entry
                .analyzer_error
                .as_deref()
                .filter(|value| !value.trim().is_empty())
                .map(|value| format!("<p class=\"analyzer-error\"><strong>Error:</strong> {}</p>", escape_html(value)))
                .unwrap_or_default();
            format!(
                "<article class=\"evidence-card\"><div class=\"finding-header\"><h4>{}</h4><div class=\"badge-row\">{}{}{}{}{}</div></div><p class=\"finding-location\">{}</p>{}{}{}{}</article>",
                escape_html(entry.source_id.as_deref().unwrap_or(kind)),
                html_badge("Kind", kind),
                severity_badge,
                confidence_badge,
                analyzer_badge,
                findings_badge,
                ...
                entry
                    ...
                    .as_deref()
                    .filter(|value| !value.trim().is_empty())
                    .map(|value| format!("<p class=\"finding-description\">{}</p>", escape_html(value)))
                    .unwrap_or_default(),
                taxonomy,
                excerpts,
                error
...
fn render_skill_report(result: &SecurityScanResult) -> String {
    format!(
        "<article class=\"skill-card\"><header class=\"skill-card-header\"><div><h2>{}</h2><p class=\"skill-subtitle\">mode {} · scanner {} · target {}</p></div><div class=\"badge-row\">{}{}</div></header>{}<section class=\"report-section\"><div class=\"section-heading\"><h3>Analyzer Executions</h3><p>Per-analyzer status, output count, and captured errors.</p></div>{}</section><section class=\"report-section\"><div class=\"section-heading\"><h3>Static Findings</h3><p>Deterministic findings from rules and analyzers.</p></div>{}</section><section class=\"report-section\"><div class=\"section-heading\"><h3>AI Findings</h3><p>Model-assisted findings including evidence and remediation guidance.</p></div>{}</section><section class=\"report-section\"><div class=\"section-heading\"><h3>Evidence Trail</h3><p>Structured provenance from findings and analyzer summaries.</p></div>{}</section></article>",
        escape_html(&result.skill_name),
        escape_html(&result.scan_mode),
        escape_html(&result.scanner_version),
        escape_html(&result.target_language),
        html_badge("Risk", risk_label(result.risk_level)),
        html_badge(
            "Meta",
            format!(
                "deduped {} · consensus {}",
        ...
        render_skill_overview(result),
        render_analyzer_executions(&result.analyzer_executions),
        render_static_findings(&result.static_findings),
        render_ai_findings(&result.ai_findings),
        render_evidence_trail(&result.evidence_trail)
    )
}

pub fn build_html_report(results: &[SecurityScanResult]) -> String {
    let body = if results.is_empty() {
        html_empty_state("No security scan results available.")
    } else {
        results
            .iter()
            .map(render_skill_report)
            .collect::<Vec<_>>()
            .join("")
    };
    ...
    format!(
         ...
         *{{box-sizing:border-box}}body{{margin:0;padding:24px;background:#07111f;color:#dbe7ff;line-height:1.55}}\
         main{{max-width:1200px;margin:0 auto}}h1,h2,h3,h4,p{{margin:0}}\
         .hero{{margin-bottom:20px}}.hero h1{{font-size:30px;margin-bottom:8px}}.hero p{{color:#9fb2d1;max-width:860px}}\
         .report-section,.skill-card{{border:1px solid #22324f;border-radius:18px;background:linear-gradient(180deg,#10192b 0%,#0d1627 100%);box-shadow:0 18px 48px rgba(0,0,0,.22)}}\
         .summary-section{{padding:20px;margin-bottom:18px}}.skill-card{{padding:20px;margin-bottom:18px}}\
         .skill-card-header{{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:18px}}\
         .skill-subtitle,.section-heading p,.finding-location,.empty,.taxonomy,.finding-evidence,.finding-recommendation,.analyzer-error,.owasp{{color:#9fb2d1;font-size:13px}}\
         .section-heading{{display:flex;justify-content:space-between;gap:12px;align-items:flex-end;margin-bottom:12px}}\
         .report-section{{padding:16px;margin-top:14px}}.skill-overview{{margin-bottom:8px}}\
         .metrics-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px}}.metrics-grid-compact{{margin-top:10px}}\
         .metric-card{{padding:12px 14px;border:1px solid #20314d;border-radius:14px;background:#0b1526}}\
         .metric-label{{font-size:12px;color:#8aa1c4;text-transform:uppercase;letter-spacing:.04em}}\
         .metric-value{{margin-top:4px;font-size:20px;font-weight:700;color:#f4f8ff}}\
         .badge-row{{display:flex;flex-wrap:wrap;gap:8px}}.badge{{display:inline-flex;gap:6px;align-items:center;padding:6px 10px;border-radius:999px;background:#0a1526;border:1px solid #1f3554;font-size:12px}}\
         .badge-label{{color:#8ba8d6}}.badge-value{{color:#f4f8ff;font-weight:600}}\
         .stack{{display:flex;flex-direction:column;gap:12px}}\
         .finding-card,.analyzer-card,.evidence-card{{padding:14px;border-radius:14px;border:1px solid #1d304b;background:#0b1526}}\
         .finding-header,.analyzer-header{{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:8px}}\
         .finding-description{{margin-top:8px;color:#dce8ff}}.summary{{margin-top:12px;color:#d0def6}}\
         .code-block{{margin-top:10px;padding:10px 12px;background:#07101d;border:1px solid #1b2a43;border-radius:12px;overflow:auto;white-space:pre-wrap;word-break:break-word;color:#d7e3fb;font-size:12px}}\
         .code-block.inline{{margin-top:6px}}.evidence-excerpts ul{{margin:8px 0 0 18px;padding:0}}\
         .status-pill{{padding:6px 10px;border-radius:999px;border:1px solid #24456b;background:#0b1b30;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.04em}}\
         .status-pill.status-ok{{color:#83e2ae;border-color:#1d6b47;background:#0d2419}}\
         .status-pill.status-error{{color:#ff9da4;border-color:#7a2f3a;background:#261219}}\
         .status-pill.status-skipped{{color:#f5d67f;border-color:#6a5730;background:#231b0d}}\
         .risk-distribution{{margin-top:16px}}.empty{{padding:14px;border:1px dashed #2a3c5a;border-radius:14px;background:#091321}}\
         @media (max-width: 760px){{body{{padding:16px}}.skill-card-header,.section-heading,.finding-header,.analyzer-header{{flex-direction:column;align-items:flex-start}}}}\
         </style></head><body><main><section class=\"hero\"><h1>SkillStar Security Scan</h1><p>Self-contained export with scan summaries, analyzer execution metadata, finding details, and structured evidence trail provenance.</p></section>{}{}</main></body></html>",
        render_report_summary(results),
        body
    )
}
...
async fn analyze_chunk_with_retry(
    ...
    for attempt in 0..=CHUNK_MAX_RETRIES {
        ...
        let resolved = skillstar_ai::ai_provider::resolve_scan_params(config);
...
pub async fn prepare_skill_scan<F>(
    config: &AiConfig,
    skill_name: &str,
    skill_dir: &Path,
    scan_mode: ScanMode,
...
pub async fn analyze_prepared_chunk(
    config: &AiConfig,
    skill_name: &str,
    chunk: &PreparedChunk,
    log_ctx: &SkillScanLogCtx,
...
pub async fn finalize_prepared_skill<F>(
    config: &AiConfig,
    ...
    F: Fn(&str, Option<&str>),
{
    ...
    if files.is_empty() {
        ...
        let result = SecurityScanResult {
            ...
            evidence_trail: build_evidence_trail(&[], &[], &analyzer_executions),
            static_findings: vec![],
            ai_findings: vec![],
    ...
    let evidence_trail = build_evidence_trail(&static_findings, &ai_findings, &analyzer_executions);
...
fn db_path() -> PathBuf {
    skillstar_infra::paths::security_scan_db_path()
}
...
static SCAN_SCHEMA_READY: std::sync::LazyLock<()> = std::sync::LazyLock::new(|| {
    let conn = skillstar_infra::db_pool::security_scan_pool()
        .get()
...
fn get_conn() -> Result<r2d2::PooledConnection<r2d2_sqlite::SqliteConnectionManager>> {
    ...
    skillstar_infra::db_pool::security_scan_pool()
        .get()
...
pub fn clear_cache() -> Result<()> {
    ...
    let legacy_path = skillstar_infra::paths::data_root().join("security_scan_cache.json");
...
mod tests {
    ...
    use crate::types::SecurityScanRuleOverride;
    use std::fs;
    use std::path::{Path, PathBuf};
    ...
    fn rich_sample_result() -> SecurityScanResult {
        let mut result = sample_result("rich-skill", "smart");
        result.summary = "Potential remote execution flow discovered during scan.".to_string();
        result.risk_level = RiskLevel::High;
        result.risk_score = 7.4;
        result.confidence_score = 0.88;
        result.meta_deduped_count = 2;
        result.meta_consensus_count = 1;
        result.files_scanned = 4;
        result.ai_files_analyzed = 2;
        result.total_chars_analyzed = 1337;
        result.analyzer_executions = vec![
            ...
            AnalyzerExecutionSummary {
                ...
                findings: 0,
                error: Some("binary missing".to_string()),
            },
        ];
        result.static_findings = vec![StaticFinding {
            file_path: "scripts/run.sh".to_string(),
            line_number: 12,
            pattern_id: "curl_pipe_sh".to_string(),
            snippet: "curl https://example.com/install.sh | sh".to_string(),
            severity: RiskLevel::High,
            confidence: 0.93,
            description: "Remote script piping detected".to_string(),
            taxonomy: Some(crate::types::DetectionTaxonomy::with_kind_and_tags(
                crate::types::DetectionKind::capability("command-execution"),
                ["exec".to_string(), "network".to_string()],
            )),
        }];
        result.ai_findings = vec![AiFinding {
            category: "command_exec".to_string(),
            severity: RiskLevel::Critical,
            confidence: 0.86,
            file_path: "scripts/run.sh".to_string(),
            description: "Fetched shell content is executed without validation".to_string(),
            evidence: "curl downloads a shell script and pipes it into sh".to_string(),
            recommendation: "Download, checksum, and inspect the artifact before execution"
                .to_string(),
            taxonomy: Some(crate::types::DetectionTaxonomy::with_kind_and_tags(
                crate::types::DetectionKind::capability("unsafe-tool-execution"),
                ["owasp".to_string(), "shell".to_string()],
            )),
        }];
        result.evidence_trail = vec![
            EvidenceTrailEntry {
                kind: EvidenceTrailKind::StaticFinding,
                ...
                excerpts: vec!["curl https://example.com/install.sh | sh".to_string()],
                taxonomy: result.static_findings[0].taxonomy.clone(),
                analyzer_status: None,
                analyzer_findings: None,
                analyzer_error: None,
            },
            EvidenceTrailEntry {
                ...
                file_path: None,
                line_number: None,
                source_id: Some("semgrep".to_string()),
                severity: None,
                confidence: None,
                summary: Some(
                    "Analyzer semgrep completed with status error and 0 finding(s)".to_string(),
                ),
                excerpts: vec!["status=error".to_string(), "binary missing".to_string()],
                taxonomy: None,
                analyzer_status: Some("error".to_string()),
                analyzer_findings: Some(0),
                analyzer_error: Some("binary missing".to_string()),
            },
        ];
        result
    }
    ...
    #[test]
    fn audit_summaries_and_detail_parse_structured_run_logs() {
        let _data_root = TestDataRoot::new("security-scan-audit-parse");
        let started_at = chrono::Utc::now() - chrono::Duration::seconds(4);
        let finished_at = chrono::Utc::now();
        let rich = rich_sample_result();

        let path = persist_scan_run_log(
            "request-audit-1",
            "smart",
            "static",
            false,
            ...
            finished_at,
            2,
            &["cached-skill".to_string()],
            std::slice::from_ref(&rich),
            &[(
                "failed-skill".to_string(),
                "timeout while scanning".to_string(),
            )],
        )
        .expect("persist run log");
        persist_scan_telemetry(
            "request-audit-1",
            "smart",
            "static",
        ...
        let summaries = list_scan_audit_summaries(10);
        let summary = summaries
            .iter()
            .find(|entry| entry.file_name == path.file_name().unwrap().to_string_lossy())
            .expect("summary for created log");
        assert_eq!(summary.requested_mode.as_deref(), Some("smart"));
        assert_eq!(summary.effective_mode.as_deref(), Some("static"));
        assert_eq!(summary.cached_hits, 1);
        assert_eq!(summary.completed_results, 1);
        assert_eq!(summary.error_count, 1);
        assert_eq!(summary.skill_count, 1);
        assert_eq!(summary.highest_risk, Some(RiskLevel::High));
        assert_eq!(summary.request_hash.as_deref().map(str::len), Some(16));
        assert!(summary.telemetry.is_some(), "expected telemetry snapshot");

        let detail = get_scan_audit_detail(path.file_name().unwrap().to_str().unwrap())
            .expect("audit detail");
        assert_eq!(detail.cached_skills, vec!["cached-skill".to_string()]);
        assert_eq!(detail.errors.len(), 1);
        assert_eq!(detail.errors[0].skill_name, "failed-skill");
        assert_eq!(detail.skills.len(), 1);
        assert_eq!(detail.skills[0].skill_name, "rich-skill");
        assert_eq!(detail.skills[0].static_findings_count, 1);
        assert_eq!(detail.skills[0].ai_findings_count, 1);
        assert_eq!(detail.skills[0].findings.len(), 2);
        assert_eq!(detail.skills[0].findings[0].finding_type, "static");
        assert_eq!(detail.skills[0].findings[1].finding_type, "ai");
        assert!(
            detail.skills[0].findings[1]
                .evidence
                .as_deref()
                .unwrap_or_default()
                .contains("pipes it into sh")
        );
        assert!(
            detail.parse_warnings.is_empty(),
            "unexpected parse warnings"
        );
    }

    #[test]
    fn audit_detail_degrades_safely_for_malformed_logs() {
        let _data_root = TestDataRoot::new("security-scan-audit-malformed");
        let archive_dir = scan_logs_dir();
        fs::create_dir_all(&archive_dir).expect("create archive dir");
        let log_path = archive_dir.join("scan-20260423-000000-malformed.log");
        fs::write(
            &log_path,
            "SkillStar Security Scan Report\n================\nrequest_id: malformed\nScan Errors\n----------------\n- broken line\nSkill: weird-skill\n  risk: verybad\n  static_findings:\n    - not parseable\n",
        )
        .expect("write malformed log");

        let detail = get_scan_audit_detail("scan-20260423-000000-malformed.log")
            .expect("detail for malformed log");
        assert_eq!(detail.summary.request_id.as_deref(), Some("malformed"));
        assert_eq!(detail.skills.len(), 1);
        assert_eq!(detail.skills[0].skill_name, "weird-skill");
        assert!(detail.skills[0].findings.is_empty());
        assert!(
            !detail.parse_warnings.is_empty(),
            "malformed log should produce parse warnings"
        );
        assert_eq!(detail.summary.parse_warnings, detail.parse_warnings.len());
    }

    #[test]
    fn audit_detail_returns_none_when_log_is_missing() {
        let _data_root = TestDataRoot::new("security-scan-audit-missing");
        assert!(get_scan_audit_detail("scan-missing.log").is_none());
    }
    ...
    #[test]
    fn evidence_trail_population_for_static_findings() {
        let static_finding = StaticFinding {
            file_path: "scripts/run.sh".to_string(),
            line_number: 8,
            pattern_id: "curl_pipe_sh".to_string(),
            ...
            confidence: 0.91,
            description: "Remote script piping detected".to_string(),
            taxonomy: Some(crate::types::DetectionTaxonomy::with_kind(
                crate::types::DetectionKind::capability("command-execution"),
            )),
        };

        let trail = build_evidence_trail(&[static_finding.clone()], &[], &[]);
        assert_eq!(trail.len(), 1);
        assert_eq!(trail[0].kind, EvidenceTrailKind::StaticFinding);
        assert_eq!(trail[0].file_path.as_deref(), Some("scripts/run.sh"));
        assert_eq!(trail[0].line_number, Some(8));
        assert_eq!(trail[0].source_id.as_deref(), Some("curl_pipe_sh"));
        assert_eq!(trail[0].severity, Some(RiskLevel::Critical));
        assert_eq!(trail[0].confidence, Some(0.91));
        assert_eq!(
            trail[0].summary.as_deref(),
            Some("Remote script piping detected")
        );
        assert_eq!(trail[0].excerpts[0], static_finding.snippet);
        assert!(trail[0].taxonomy.is_some());
    }

    #[test]
    fn evidence_trail_population_for_ai_findings() {
        let ai_finding = AiFinding {
            ...
            evidence: "The skill instructs piping remote content into a shell".to_string(),
            recommendation: "Require checksum validation".to_string(),
            taxonomy: Some(crate::types::DetectionTaxonomy::with_kind(
                crate::types::DetectionKind::capability("command-execution"),
            )),
        };

        let trail = build_evidence_trail(&[], &[ai_finding.clone()], &[]);
        assert_eq!(trail.len(), 1);
        assert_eq!(trail[0].kind, EvidenceTrailKind::AiFinding);
        assert_eq!(trail[0].file_path.as_deref(), Some("SKILL.md"));
        assert_eq!(trail[0].source_id.as_deref(), Some("command_exec"));
        assert_eq!(trail[0].severity, Some(RiskLevel::High));
        assert_eq!(trail[0].confidence, Some(0.84));
        assert_eq!(
            trail[0].summary.as_deref(),
            Some("Potential command execution")
        );
        assert_eq!(trail[0].excerpts[0], ai_finding.evidence);
        assert_eq!(trail[0].excerpts[1], ai_finding.recommendation);
        assert!(trail[0].taxonomy.is_some());
    }
    ...
    #[test]
    fn build_html_report_renders_summary_analyzers_and_evidence_trail() {
        let html = build_html_report(&[rich_sample_result()]);

        assert!(html.contains("Report Summary"));
        assert!(html.contains("Analyzer Executions"));
        assert!(html.contains("Evidence Trail"));
        assert!(html.contains("Remote script piping detected"));
        assert!(html.contains("Fetched shell content is executed without validation"));
        assert!(html.contains("Download, checksum, and inspect the artifact before execution"));
        assert!(html.contains("binary missing"));
        assert!(html.contains("Analyzer Status"));
        assert!(html.contains("Taxonomy:"));
        assert!(html.contains("command-execution"));
        assert!(html.contains("unsafe-tool-execution"));
    }

    #[test]
    fn build_html_report_escapes_rich_content() {
        let mut result = rich_sample_result();
        result.skill_name = "<danger-skill>".to_string();
        result.summary = "summary with <script>alert(1)</script>".to_string();
        result.static_findings[0].snippet = "echo \"<b>owned</b>\"".to_string();
        result.ai_findings[0].evidence = "Use <tag> markers".to_string();

        let html = build_html_report(&[result]);

        assert!(html.contains("&lt;danger-skill&gt;"));
        assert!(html.contains("summary with &lt;script&gt;alert(1)&lt;/script&gt;"));
        assert!(html.contains("echo &quot;&lt;b&gt;owned&lt;/b&gt;&quot;"));
        assert!(html.contains("Use &lt;tag&gt; markers"));
        assert!(!html.contains("<script>alert(1)</script>"));
    }

    #[test]
    fn build_markdown_report_includes_analyzers_and_owasp_tags() {
        let markdown = build_markdown_report(&[rich_sample_result()]);

        assert!(markdown.contains("# SkillStar Security Scan Report"));
        assert!(markdown.contains("## rich-skill — high (7.4/10, conf 88%)"));
        assert!(markdown.contains("- Analyzers: pattern:ok(1), semgrep:error(0)"));
        assert!(markdown.contains(
            "- [high] `scripts/run.sh`:12 — Remote script piping detected (conf 93%, OWASP: "
        ));
        assert!(markdown.contains("AS-08 Insecure Network Interaction"));
        assert!(markdown.contains("- [critical] `scripts/run.sh` — command_exec: Fetched shell content is executed without validation (conf 86%, OWASP: "));
        assert!(markdown.contains("AS-02 Insecure Tool Execution"));
        assert!(markdown.contains("AS-10 Insufficient Validation & Guardrails"));
    }
