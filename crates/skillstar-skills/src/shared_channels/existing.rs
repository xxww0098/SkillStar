...
use crate::git::transport::GitOperationPhase;
...
impl ExistingRepositoryScanner for GitExistingRepositoryScanner {
    fn scan(
        ...
        let discovered = crate::discovery::discover_skills(&repository_dir, true);
...
fn collect_repository_files(
    ...
    let mut files = crate::git::ops::list_tree_paths(root).map_err(|_| {
        if session.is_cancelled() {
            cancelled_error()
        } else {
            inventory_io_error()
...
fn materialize_all_skill_directories(
    ...
    if !directories.is_empty() {
        crate::git::ops::apply_sparse_checkout_in_session(repository_dir, &directories, session)
            .map_err(|_| {
                if session.is_cancelled() {
                    cancelled_error()
                } else {
...
mod tests {
    ...
    fn sparse_checkout_inventory_uses_the_tracked_tree_and_materializes_every_skill() {
        ...
        let skills = crate::discovery::discover_skills(&checkout, true);
