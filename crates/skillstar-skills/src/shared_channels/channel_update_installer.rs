...
fn rollback_exact(receipt: &ChannelSkillUpdateReceipt) -> Result<(), SharedChannelError> {
    ...
    let current_repository_url = crate::lockfile::Lockfile::load(&crate::lockfile::lockfile_path())
        .ok()
        .and_then(|lockfile| {
            lockfile
                .skills
    ...
    crate::repo_scanner::scan_install::install_from_repo_at_with_source_migrations(
        checkout,
