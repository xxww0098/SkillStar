...
pub struct ChannelSkillUpdateReceipt {
    ...
    pub previous_lock_entry: crate::lockfile::LockEntry,
    ...
    I: ChannelSubscriptionInstaller + ChannelSubscriptionUpdater,
{
    ...
    pub async fn check_update(
        ...
        let (channel, repository, manifest) = match remote {
            Ok(value) => value,
            Err(error) => {
                let remote_state_changed = super::subscription_remote::mark_remote_failure(
...
fn validate_resolutions(
    ...
    for item in resolutions {
        crate::content::validate_skill_name(&item.skill_id).map_err(|_| {
            SharedChannelError::new(
                SharedChannelErrorCode::SubscriptionSelectionInvalid,
