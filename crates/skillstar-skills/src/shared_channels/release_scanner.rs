...
pub struct GitChannelDraftScanner {
    facade: crate::git_skill::GitSkillFacade,
}

impl GitChannelDraftScanner {
    pub fn new(facade: crate::git_skill::GitSkillFacade) -> Self {
        Self { facade }
    }
}
...
impl ChannelDraftScanner for GitChannelDraftScanner {
    fn scan(
        ...
        crate::git::transport::execute_remote_command(
            &mut command,
            None,
            &[
                "clone",
        ...
        session.emit(
            crate::git::transport::GitOperationPhase::Running,
            &repository.clone_url,
        );
        let commit_sha = crate::git::ops::rev_parse(&repository_dir, "HEAD")
            .map_err(|_| scanner_error(session))?;
        ...
        if session.is_cancelled() {
            session.emit(
                crate::git::transport::GitOperationPhase::Cancelled,
                &repository.clone_url,
            );
            return Err(cancelled_error());
        }
        session.emit(
            crate::git::transport::GitOperationPhase::Completed,
            &repository.clone_url,
        );
        Ok(snapshot)
    }
...
fn snapshot_published_tree(
    ...
    let entries = crate::git::ops::list_tree_entries_at(repository_dir, commit_sha)
        .map_err(|_| scanner_error(session))?;
    ...
    crate::git::transport::configure_remote_command(&mut command, remote, session)
        .map_err(|_| scanner_error(session))?;
    command
        .arg("archive")
        .arg("--format=tar")
    ...
    crate::git::transport::configure_process_group(&mut command);
    ...
    let status = loop {
        if unpack_result.is_none() {
            match unpack_receiver.try_recv() {
                ...
                Ok(Err(error)) => {
                    crate::git::transport::terminate_child_tree(&mut child);
                    let _ = child.wait();
        ...
        if session.is_cancelled() {
            ...
            let _ = unpack_reader.join();
            let _ = stderr_reader.join();
            session.emit(crate::git::transport::GitOperationPhase::Cancelled, remote);
            return Err(cancelled_error());
        }
    ...
    let discovered =
        crate::discovery::discover_skills_without_dedup(&extraction, true, Some(repository_name));
    ...
    for skill in discovered {
        ...
        let snapshot = crate::content::snapshot_path(&skill.id, &root).map_err(|_| {
...
fn scanner_error(session: &crate::git::transport::GitOperationSession) -> SharedChannelError {
    if session.is_cancelled() {
        cancelled_error()
    } else {
...
mod tests {
    ...
    fn exact_commit_snapshot_uses_the_shared_known_content_hash_and_ignores_untracked_files() {
        ...
        let commit = crate::git::ops::rev_parse(&repo, "HEAD").unwrap();
    ...
    fn exact_commit_snapshot_disables_export_ignore_and_export_subst() {
        ...
        let commit = crate::git::ops::rev_parse(&repo, "HEAD").unwrap();
        let expected = crate::content::snapshot_path("writer", &repo.join("skills/writer"))
            .unwrap()
            .content_hash;
    ...
    fn scanner_uses_the_api_default_branch_in_an_isolated_checkout() {
        ...
        let scanner = GitChannelDraftScanner::new(crate::git_skill::GitSkillFacade::new(
            crate::git::transport::GitOperationSession::public(),
        ));
