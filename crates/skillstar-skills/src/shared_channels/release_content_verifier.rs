...
pub(super) fn verify_release_content_blocking(
    ...
    let _guard = crate::skill_update::acquire_update_transaction_lock().map_err(|error| {
    ...
    let verification_source = format!(
        "channel-verify-{}-{}",
        repository.id,
        crate::repo_scanner::cache_dir_name(&repository.clone_url)
    );
    let repo_dir = crate::repo_scanner::clone_or_fetch_repo_at_in_session(
...
fn verify_release_checkout(
    ...
    for skill in
        crate::discovery::discover_skills_without_dedup(repo_dir, true, Some(default_skill_id))
    {
    ...
    for skill in released {
        ...
        let snapshot = crate::content::snapshot_path(&skill.id, &target_root)
...
fn release_content_git_error(error: anyhow::Error) -> SharedChannelError {
    use crate::git::transport::GitTransportErrorCode;
...
fn verify_checkout_head(repo_dir: &Path, expected: &str) -> Result<(), SharedChannelError> {
    let head =
        crate::git::ops::rev_parse(repo_dir, "HEAD").map_err(|_| content_integrity_error())?;
...
mod tests {
    use super::*;
    use crate::git::transport::{GitTransportError, GitTransportErrorCode};
    use std::fs;
    ...
    fn exact_checkout_verifier_rejects_extra_missing_and_modified_skill_content() {
        ...
        let snapshot = crate::content::snapshot_path("writer", &skill_root).unwrap();
