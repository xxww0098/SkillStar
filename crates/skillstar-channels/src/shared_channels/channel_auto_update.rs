    ...
    I: ChannelSubscriptionInstaller + ChannelSubscriptionUpdater,
{
    ...
    pub(super) async fn claim_due_auto_updates(
        ...
        for subscription in &mut store.subscriptions {
            // Disabled subscriptions are still claimed and still checked; only
            // *applying* is off (see the `Checked` early return in
            // `execute_auto_update`). Skipping the claim entirely meant a
            // disabled subscription was never probed, and since channel Skills
            // are excluded from patrol and forced to `update_available: false`,
            // `check_update` is their only source of update state — so the user
            // was never told a new release existed at all.
            //
            // The reason the claim used to be skipped (a failed probe flipping
            // `remote_state` and freezing manual actions for up to an hour) is
            // handled at the probe itself: `execute_auto_update` calls
            // `probe_update` for disabled subscriptions, which records the
            // failure in the snapshot without touching `remote_state`.
            if subscription
                .auto_update
                .last_run
                .as_ref()
        ...
        F: Fn() -> DateTime<Utc> + Send + Sync,
    {
        ...
        // An unattended check on a subscription the user switched off must not
        // be able to degrade `remote_state` and freeze their manual actions.
        let snapshot = if previous.auto_update.enabled {
            self.check_update(repository_id).await
        } else {
            self.probe_update(repository_id).await
        };
