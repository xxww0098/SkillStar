...
async fn auto_update_is_opt_in_and_the_preference_survives_restart() {
    ...
    // Disabled means "do not apply", not "do not look". `check_update` is the
    // only writer of channel update state — channel Skills are excluded from
    // patrol and forced to `update_available: false` — so skipping the check
    // would leave the user with no way to learn a new release exists.
    ...
    // Enabled + due → exactly one run. The clean "reader" skill is applied
    // (it can be proven unmodified against its baseline); the newly-added
...
#[tokio::test]
async fn background_probe_of_a_disabled_subscription_never_degrades_remote_state() {
    let (gateway, subscriptions, installer) = fixtures();
    let app = service(gateway.clone(), subscriptions.clone(), installer.clone());
    assert!(!app.auto_update_state(42).unwrap().enabled);

    // A transient network failure during an unattended probe must not set
    // remote_state: `ensure_remote_access` reads it to freeze the user's manual
    // apply/install/rollback actions, so a background task would otherwise lock
    // them out of a channel they never asked it to touch.
    *gateway.offline.lock().unwrap() = true;
    let executions = app.run_due_auto_updates().await.unwrap();
    assert_eq!(executions.len(), 1);
    assert_eq!(
        executions[0].run.status,
        ChannelAutoUpdateRunStatus::RetryableFailure,
        "a failed probe is still reported as a failed run"
    );
    assert_eq!(
        ...
        "an unattended probe must not freeze the user's manual operations"
    );

    // The same failure on an *enabled* subscription is the user's own automation
    // acting on their behalf, so it does record the degraded state.
    app.set_auto_update_enabled(42, true).await.unwrap();
    app.run_due_auto_updates().await.unwrap();
    assert_eq!(
        subscriptions.store.lock().unwrap().subscriptions[0]
            .remote_state
            .status,
...
async fn baseline_snapshot_and_unresolved_failures_stay_paused() {
    ...
    installer.divergence_reasons.lock().unwrap().insert(
        "reader".into(),
        skillstar_skills::skill_update::LocalDivergenceReason::BaselineMissing,
    );
    installer.divergence_reasons.lock().unwrap().insert(
        "writer".into(),
        skillstar_skills::skill_update::LocalDivergenceReason::SnapshotFailed,
    );
    make_due(&subscriptions, now);
