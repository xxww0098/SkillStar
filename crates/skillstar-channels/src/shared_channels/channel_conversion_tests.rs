//! Channel conversion: `convert_to_local_and_commit` must remove the
//! installation safety copy when the metadata commit rolls back.
//! (Moved from `skillstar-skills::skill_install_removal_tests` when the
//! shared-channel domain split into `skillstar-channels`.)
