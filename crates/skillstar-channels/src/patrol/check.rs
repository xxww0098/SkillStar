...
pub fn check_skill_update_local_in_session(
    ...
    match skillstar_skills::git::ops::check_update_in_session(skill_path, session) {
        ...
        Err(err) => {
            ...
            // A failed check is "unknown", not "up to date". `Some(false)` here
            // let a single offline patrol cycle overwrite a real update badge
            // with `false` and persist it; `None` makes the caller skip the
            // skill and keep whatever the last successful scan recorded.
            None
        }
    }
}
...
pub fn collect_hub_skills() -> Result<Vec<HubSkillEntry>> {
    ...
    for entry in entries.flatten() {
        ...
        // Skips empty names and staging residue from interrupted hub writes,
        // which would otherwise be patrolled as if they were installed Skills.
        if !skillstar_skills::hub_entry::is_managed_hub_entry(&name) {
            continue;
        }
