...
mod tests {
    ...
    #[test]
    fn extract_json_payload_finds_array() {
        let raw = "Some text before [{\"name\":\"a\"}] after";
        assert_eq!(extract_json_payload(raw), "[{\"name\":\"a\"}]");
    }

    #[test]
    fn extract_json_payload_finds_wrapped_object() {
        let raw = "Here is the result: {\"recommendations\":[{\"name\":\"a\"}]}";
        assert_eq!(
    ...
    #[test]
    fn extract_json_payload_fallback_to_trimmed() {
        let raw = "  just text  ";
        assert_eq!(extract_json_payload(raw), "just text");
    }

    #[test]
    fn default_skill_pick_score_decreases_with_rank() {
        assert_eq!(default_skill_pick_score(0), 80);
        assert_eq!(default_skill_pick_score(1), 74);
        assert_eq!(default_skill_pick_score(4), 56);
        assert_eq!(default_skill_pick_score(5), 55);
        assert_eq!(default_skill_pick_score(10), 55);
    }

    #[test]
    fn fallback_skill_pick_rank_score_decreases_with_rank() {
        assert_eq!(fallback_skill_pick_rank_score(0), 82);
        assert_eq!(fallback_skill_pick_rank_score(1), 78);
        assert_eq!(fallback_skill_pick_rank_score(10), 42);
    }

    #[test]
    fn extract_match_tokens_filters_low_signal_and_short() {
        let tokens = extract_match_tokens("Build a Next.js app for the web");
        assert!(tokens.contains(&"next.js".to_string()));
        assert!(tokens.contains(&"next".to_string()));
    ...
    #[test]
    fn compute_local_skill_pick_score_boosts_name_match() {
        let candidate = SkillPickCandidate {
            name: "nextjs-frontend".to_string(),
            description: "A generic frontend skill".to_string(),
        };
        let prompt_lower = "build a nextjs frontend".to_lowercase();
        let prompt_tokens: std::collections::HashSet<String> =
            extract_match_tokens("build a nextjs frontend")
                .into_iter()
                .collect();
        let score = compute_local_skill_pick_score(&prompt_lower, &prompt_tokens, &candidate);
        assert!(score > 50, "direct name match should produce a high score");
    }

    #[test]
    fn compute_local_skill_pick_score_zero_for_unrelated() {
        let candidate = SkillPickCandidate {
    ...
    #[test]
    fn normalize_skill_pick_reason_collapses_whitespace() {
        assert_eq!(
