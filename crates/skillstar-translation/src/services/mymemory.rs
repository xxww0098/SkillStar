...
impl MyMemoryService {
    ...
    fn get_mymemory_de() -> String {
        use std::fs;
        let path = skillstar_infra::paths::mymemory_disabled_path();
...
impl TranslationProvider for MyMemoryService {
    ...
    fn translate(
        ...
        async move {
            ...
            let url =
                reqwest::Url::parse_with_params("https://api.mymemory.translated.net/get", &params)
            ...
            let resp =
                tokio::time::timeout(std::time::Duration::from_secs(15), client.get(url).send())
                    ...
                    .map_err(|e| {
                        TranslationError::HttpError("MyMemory".to_string(), e.to_string())
                    })?;
            ...
            if translated.is_empty() {
                return Err(TranslationError::Unknown(
                    "MyMemory returned empty translation".to_string(),
                ));
            }
