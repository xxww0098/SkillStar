...
pub fn export_bundle(skill_name: &str, output_path: Option<&str>) -> Result<PathBuf> {
    ...
    let effective_dir = if skillstar_infra::fs_ops::is_link(&skill_dir) {
        skillstar_infra::fs_ops::read_link_resolved(&skill_dir)
    ...
    let description = skillstar_core_types::extract_skill_description(&effective_dir);
...
pub fn import_bundle(file_path: &str, force: bool) -> Result<ImportBundleResult> {
    ...
    skillstar_security_scan::invalidate_skill_cache(&manifest.name);
