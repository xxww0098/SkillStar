use crate::{local_skill, lockfile::{self, LockEntry}, repo_scanner};
use skillstar_core_types::{Skill, SkillCategory, extract_github_source_from_url, extract_skill_description};
use skillstar_git::ops as git_ops;
use skillstar_projects::agents::{self as agent_profile, AgentProfile};
use skillstar_translation::cache::CachedTranslation;
...
pub fn installed_snapshot_markers() -> HashSet<String> {
    ...
    let hub_skills_dir = skillstar_infra::paths::hub_skills_dir();
    if let Ok(entries) = std::fs::read_dir(&hub_skills_dir) {
        for entry in entries.flatten() {
            let path = entry.path();
            if !path.is_dir() && !skillstar_infra::fs_ops::is_link(&path) {
                continue;
            }
...
pub async fn list_installed_skills() -> Result<Vec<Skill>> {
    ...
    let skill_dirs = collect_skill_dirs(&skillstar_infra::paths::hub_skills_dir(), None)?;
    ...
    let config = skillstar_ai::ai_provider::load_config_async().await;
    ...
    let preloaded_translations: Arc<HashMap<String, CachedTranslation>> =
        if let Some(ref lang) = target_language {
            ...
            let map = tokio::task::spawn_blocking(move || {
                skillstar_translation::cache::preload_short_translations(&lang_clone)
                    .unwrap_or_default()
            })
            .await
            .unwrap_or_default();
...
pub async fn refresh_skill_updates(names: Option<Vec<String>>) -> Result<Vec<SkillUpdateState>> {
    ...
    let skill_dirs = collect_skill_dirs(
        &skillstar_infra::paths::hub_skills_dir(),
        name_filter.as_ref(),
    )?;
...
fn build_installed_skill(
    ...
    if target_language.is_some() && !description.trim().is_empty() {
        let hex = skillstar_infra::util::sha256_hex(description.as_bytes());
    ...
    let skill_type = if local_skill::is_local_skill(&name) {
        skillstar_core_types::SkillType::Local
    } else {
        skillstar_core_types::SkillType::Hub
    };
...
fn detect_agent_links(skill_name: &str, profiles: &[AgentProfile]) -> Vec<String> {
    ...
    for profile in profiles {
        ...
        if skillstar_infra::fs_ops::is_link(&link_path) && link_path.exists() {
