...
use skillstar_core_types::{Skill, SkillCategory, extract_skill_description};
...
pub fn is_local_skill(name: &str) -> bool {
    ...
    let Ok(resolved) = skillstar_infra::fs_ops::read_link_resolved(&skill_path) else {
        return false;
    };

    let local_dir = skillstar_infra::paths::local_skills_dir();
...
pub fn create(name: &str, content: Option<&str>) -> Result<Skill> {
    ...
    skillstar_infra::fs_ops::create_symlink(&skill_local_path, &skill_hub_path)
    ...
    Ok(Skill {
        ...
        skill_type: skillstar_core_types::SkillType::Local,
        stars: 0,
        installed: true,
...
pub fn adopt_existing_dir(name: &str, source_dir: &Path) -> Result<PathBuf> {
    ...
    if let Err(err) = skillstar_infra::fs_ops::create_symlink(&skill_local_path, &skill_hub_path)
        .with_context(|| format!("Failed to create hub symlink for '{}'", name))
    {
...
pub fn reconcile_hub_symlinks() {
    ...
    for entry in entries.flatten() {
        ...
        if let Err(e) = skillstar_infra::fs_ops::create_symlink(&path, &hub_path) {
            warn!(
                target: "local_skill",
                skill = %name,
                error = %e,
...
pub fn delete(name: &str) -> Result<()> {
    ...
    let hub_path = hub_dir.join(name);
    if hub_path.symlink_metadata().is_ok() {
        if skillstar_infra::fs_ops::is_link(&hub_path) {
            skillstar_infra::fs_ops::remove_symlink(&hub_path)
...
pub fn migrate_existing() -> Result<u32> {
    ...
    for entry in entries.flatten() {
        ...
        if skillstar_infra::fs_ops::is_link(&path) || !meta.is_dir() {
            continue;
        }
...
fn migrate_single_skill(src: &Path, dest: &Path) -> Result<()> {
    ...
    skillstar_infra::fs_ops::create_symlink(dest, src)
