use super::*;
    ...
    fn test_activate_tool_success_claude_code() {
        ...
        let activation = activate_tool(&mut store, &created.id, "claude-code", Some("deepseek-chat"), None).unwrap();
    ...
    fn test_activate_tool_success_codex() {
        ...
        let activation = activate_tool(&mut store, &created.id, "codex", Some("deepseek-reasoner"), None).unwrap();
        assert_eq!(activation.provider_id, created.id);
    ...
    fn test_activate_tool_replaces_previous_activation() {
        ...
        activate_tool(&mut store, &created1.id, "claude-code", Some("model-a"), None).unwrap();
        let stored = store.tool_activations.get("claude-code").unwrap().as_ref().unwrap();
        assert_eq!(stored.provider_id, created1.id);

        // Activate provider B for claude-code — should replace A
        activate_tool(&mut store, &created2.id, "claude-code", Some("model-b"), None).unwrap();
        let stored = store.tool_activations.get("claude-code").unwrap().as_ref().unwrap();
    ...
    fn test_deactivate_tool_returns_previous() {
        ...
        // Activate first
        activate_tool(&mut store, &created.id, "claude-code", Some("deepseek-chat"), None).unwrap();
    ...
    fn test_activate_deactivate_round_trip() {
        ...
        // Activate
        let activation = activate_tool(&mut store, &created.id, "codex", Some("model-x"), None).unwrap();
        assert_eq!(activation.provider_id, created.id);
    ...
    fn test_activate_multiple_tools_same_provider() {
        ...
        activate_tool(&mut store, &created.id, "claude-code", Some("deepseek-chat"), None).unwrap();
        activate_tool(&mut store, &created.id, "codex", Some("deepseek-reasoner"), None).unwrap();
