...
fn test_merge_json_write_preserves_existing() {
    ...
    assert_eq!(parsed.get("key1").unwrap().as_str().unwrap(), "new_value");
...
fn test_merge_json_env_write_new_file() {
    ...
    let fields: Vec<(&str, Value)> = vec![
        ...
        ("ANTHROPIC_AUTH_TOKEN", Value::String("sk-test".to_string())),
    ];
...
fn test_merge_json_env_write_preserves_all_fields() {
    ...
    let fields: Vec<(&str, Value)> = vec![
        ("ANTHROPIC_BASE_URL", Value::String("new_url".to_string())),
        ("ANTHROPIC_MODEL", Value::String("model-x".to_string())),
    ];
    merge_json_env_write(&path, &fields).unwrap();
    ...
    assert_eq!(env.get("MY_VAR").unwrap().as_str().unwrap(), "my_value");
}
...
fn test_rolling_backup_keeps_last_5() {
    ...
    for i in 0..7u128 {
        let backup_name = format!("{}.bak.{}", config_path.to_string_lossy(), 1000 + i);
...
fn test_codex_auth_json_merge_write() {
    ...
    let fields: Vec<(&str, Value)> =
        vec![("OPENAI_API_KEY", Value::String("new-key-12345".to_string()))];
