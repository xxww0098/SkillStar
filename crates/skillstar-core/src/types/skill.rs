...
mod tests {
    ...
    #[test]
    fn classify_skill_from_stars() {
        let skill = Skill::from_skills_sh(
            "demo".into(),
            "desc".into(),
            150,
            "publisher".into(),
        ...
        assert_eq!(skill.category, SkillCategory::Popular);
        assert_eq!(skill.source.as_deref(), Some("publisher"));
    }

    #[test]
    fn parse_skill_content_reads_frontmatter() {
        let content = parse_skill_content(
            "demo".into(),
            "---\ndescription: test\ntriggers:\n  - one\nscopes:\n  - local\nallowed-tools:\n  - bash\n---\nbody".into(),
        );

        assert_eq!(content.description.as_deref(), Some("test"));
        assert_eq!(content.triggers, vec!["one"]);
        assert_eq!(content.scopes, vec!["local"]);
        assert_eq!(content.allowed_tools, vec!["bash"]);
    }
}
