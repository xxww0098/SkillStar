//! SKILL.md frontmatter parse and diagnostics.
//!
...
//! description drives agent triggering) and the `npx skills` installer (skills
//! without `name`/`description` are not valid).
//!
//! The split is deliberately two-tier, mirroring the ecosystem:
//! - **Blocking** issues make a skill un-installable. These are the fields an
//!   agent actually consumes (`description`) or names that exceed the spec cap.
//! - **Advisory** issues (`name` missing — callers typically fall back to the
//!   directory name — or not kebab-case) are reported but never block.
